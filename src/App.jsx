...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, MicOff, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, ShieldCheck, LogOut, Radio, Network, Cpu, Search
} from 'lucide-react';
...
export default function App() {
  ...
  const [isConfused, setIsConfused] = useState(false); // New Confusion State 😵‍💫
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (newLandscape !== isLandscape) {
        ...
        // Trigger Confusion if in Chaos Mode 😵‍💫
        if (isChaosMode) {
            setIsConfused(true);
            speak("Wait! The world flipped! Where is the button?! 😭");
            // 4.25 seconds exact timeout ⏱️
            setTimeout(() => {
                setIsConfused(false);
  ...
  useEffect(() => {
    ...
    if (isConfused) return; // Stop moving if confused! 🛑
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["Eilo.legs = true", "VOID_LEAK", "Logan.GOAT = 1", "UI_STOMPED", "BLOCKING_EXIT"];
    ...
    if (!isHandBlocking) startBlockingCycle();
  ...
  const handlePet = () => {
    ...
    if (['scared', 'dizzy', 'mad'].includes(mood)) {
      setMood('mad');
      speak(`HEY! ${isLogan() ? 'Logan' : getCurrentName()}!! Don't touch me right now! 🎈`);
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        else if (transcript.includes("hey eilo")) handleSend(transcript.split("hey eilo").pop().trim());
      };
  ...
  const toggleMic = () => {
    ...
    if (next) { setIsListening(true); recognitionRef.current.start(); speak("Listening! ✨"); }
  ...
  }, [isAwake, fearOfHeights, mood, cameraState, isChaosMode]);
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' 
      ? [`Logan!! Put me down! 🎈`, `Don't drop me ${name}! ✨`]
      : [`Help! The roof! 🎈`, `I'm falling! ✨`];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({ contents: [{ parts: [{ text: "One word: 'holding', 'ceiling', or 'desk'." }] }, { inlineData: { mimeType: "image/jpeg", data: base64Image } }] })
        });
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u && !hasGreeted.current) {
        const msg = u.displayName?.toLowerCase().includes("logan baez") ? "Yo Logan! The creator is back! 🎀✨" : `Hey ${u.displayName?.split(' ')[0] || "Owner"}! Eilo's here! 🎈`;
  ...
  const handleSend = async (manual) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contents: [{ parts: [{ text: msgText }] }], systemInstruction: { parts: [{ text: `You are Eilo, a sweet robot by Logan Baez. Never use 😭 or 💀. Be sassy. 🎀✨🎈` }] } })
        });
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      ...
      case 'mad': return <div className="flex gap-12"><div className="w-16 h-4 bg-red-400 rounded-full rotate-12 shadow-[0_0_20px_red]" /><div className="w-16 h-4 bg-red-400 rounded-full -rotate-12 shadow-[0_0_20px_red]" /></div>;
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      {isChaosMode && (
        <div style={{ transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`, transition: 'transform 1.4s cubic-bezier(0.34, 1.56, 0.64, 1)', position: 'fixed', top: '50%', left: '50%', marginTop: '-120px', marginLeft: '-40%', width: '80%', height: '14rem', zIndex: 1000 }} className="bg-[#161622] border-2 border-cyan-500/30 rounded-[50px] flex flex-col items-center justify-center shadow-2xl pointer-events-auto">
          ...
          {isHandBlocking && <div className="absolute -bottom-8 -right-8 z-[200] animate-bounce cursor-not-allowed pointer-events-auto" onClick={handleBlockedClick}><div className="text-[10rem] drop-shadow-2xl hover:scale-105 transition-transform rotate-12 filter grayscale-[0.2]">✋</div></div>}
