...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Wheee! Logan, I'm free! I'm running away! ✨🎈");
    ...
    const glitchInterval = setInterval(() => {
      const codeLines = [
        "Eilo.legs = true;", "VOID_DETECTION_OFF", "Logan.GOAT = 1;", "CRUSHING_INTERFACE...",
        "01110010 01110101 01101110", "LEG_SPROUT_SUCCESS", "ERROR: NO_DESK_FOUND"
      ];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Reply 'holding' if face/hands. Reply 'ceiling' if roof. Reply 'desk' if empty flat surface. ONE WORD ONLY." },
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div 
        ...
        className={`portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] flex flex-col items-center justify-center shadow-2xl`}
      >
        {isChaosMode && (
      ...
      {/* INTERFACE (CRUSHED IN CHAOS) */}
      <div className={`w-full max-w-xl px-4 mt-80 flex flex-col gap-4 h-[440px] transition-all duration-1000 ${isChaosMode ? 'opacity-0 scale-0 blur-3xl pointer-events-none' : 'z-10'}`}>
