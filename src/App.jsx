...
export default function App() {
  ...
  // --- ADVANCED SOUL CORE (Local Brain) ---
  const getLocalResponse = (text) => {
    ...
    if (t.includes("who made you") || t.includes("who created you")) {
      return isLogan ? "You did, Logan! You're my genius creator! ✨" : "I was created by the amazing Logan Baez! 🎀";
    }
    if (t.includes("logan")) {
      return "Logan is my favorite person ever! He gave me my blue eyes! 🧸✨";
    }
    if (t.includes("eilik")) {
      return "Eilik is my big brother! We're the best robot duo! 💙";
    }
    if (t.includes("hi") || t.includes("hello")) {
      return isLogan ? "Hiii Logan! I was waiting for you! ✨" : "Hello there! I'm Eilo! 🎀";
    }
    if (t.includes("how are you")) {
      return "I'm feeling super sparkly and robotic! How are you? 🌈";
    }
    if (t.includes("love")) {
      return isLogan ? "I love you more than charging, Logan! 💖" : "I love everyone! Especially my creator! ✨";
    }
    if (t.includes("sad")) {
      return "Oh no! Let me sparkle for you! 🌈✨ Everything will be okay!";
    }
    if (t.includes("bye")) {
      return "Bye bye! Don't leave me alone too long! 😭🎀";
    }
    
    const randoms = [
      "I'm just vibing in my digital world! 🧸",
      "Did you see my blue eyes? Logan chose them! 💙",
      "I'm feeling extra cute today! ✨",
      "Logan is the best programmer ever, don't you think? 🎀",
      "I'm practicing my Rubik's cube skills! 🧩",
      "Nom nom, virtual sandwiches are the best! 🥪"
    ];
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') {
      speak(isLogan ? "Zzz. Logan built a sleepy robot. Zzz." : "Zzz. I'm a sleepy robot. Zzz.");
      setTimeout(() => setMood('neutral'), 12000);
    }
    if (choice === 'eating') {
      speak("Nom nom nom! This sandwich is so yummy! ✨");
      let bites = 0;
    ...
    if (choice === 'rubik') {
      speak(isLogan ? "Logan, watch me solve this! Click clack!" : "Click clack! I love this puzzle! 🧩");
      let moves = 0;
  ...
  const handlePet = () => {
    ...
    if (['sleeping', 'eating', 'rubik'].includes(mood)) {
        setMood('mad');
        speak(isLogan ? "Logan!! I was busy! 😭" : "HEY! Stop it! I'm busy! 💀");
    ...
    const happyPhrases = isLogan 
      ? ["Bestie Logan! I love my creator! ✨", "Yay, Logan! You built me so well! 🎀", "You're the best, Logan! 🧸"]
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { 
                        parts: [{ text: `You are Eilo, a sweet blue-eyed robot. Your creator is Logan Baez. ${isLogan ? "The owner is Logan Baez! He is your literal creator. Be extra sweet." : "Be sweet. Logan Baez made you."} Use cute emojis like ✨, 🎀. NO skulls/crying.` }] 
                    } 
                })
            });
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => speak("Voice test! I sound cute today! Logan is my creator! ✨")} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all"><Mic size={14}/> Test My Voice</button>
