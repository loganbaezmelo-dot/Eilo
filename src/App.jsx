...
import { 
  ...
  Send, Battery, Wifi, Cpu, Zap, MessageSquare, Settings, X, Key, LogOut, Hand, Coffee
} from 'lucide-react';
...
export default function App() {
  ...
  const [mood, setMood] = useState('neutral'); // neutral, happy, thinking, sleeping, eating, rubik, mad
  ...
  // --- AUDIO ENGINE ---
  const speak = async (text, style = "cute") => {
  ...
  const resetIdleTimer = () => {
    ...
    if (mood !== 'neutral' && mood !== 'happy' && mood !== 'thinking' && mood !== 'mad') {
        setMood('neutral');
    }
    idleTimerRef.current = setTimeout(triggerIdleAction, 15000); // 15 seconds to idle
  };

  const triggerIdleAction = () => {
    if (!isAwake || isThinking) return;
    const actions = ['sleeping', 'eating', 'rubik'];
    const choice = actions[Math.floor(Math.random() * actions.length)];
    setMood(choice);
    
    if (choice === 'sleeping') speak("Zzz... Honk... Mimimi... Zzz...");
    if (choice === 'eating') speak("Nom nom, crunch crunch! So yummy! ✨");
    if (choice === 'rubik') speak("Click, clack, click, rotate! Almost got it! 🎈");
  };

  const handlePet = () => {
    if (!isAwake) return;
    
    // Check if Eilo was busy in Idle mode
    if (['sleeping', 'eating', 'rubik'].includes(mood)) {
        setMood('mad');
        speak("HEY! Don't touch me, I was busy! ✨");
    ...
    setMood('happy');
    const phrases = ["That feels so nice! ✨", "Hehe, I love pets! 🎀", "You're so kind! 🧸"];
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({
                    ...
                    systemInstruction: { parts: [{ text: "You are Eilo, a sweet, energetic robot with blue eyes. You love your owner. NO skulls or crying." }] }
                })
            });
            const data = await response.json();
    ...
    switch (mood) {
      ...
      case 'sleeping': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full">
          ...
          <div className="w-24 h-3 bg-cyan-600 rounded-full shadow-[0_0_20px_rgba(8,145,178,0.5)]" />
          <div className="absolute top-1/4 right-1/4 text-cyan-400 text-2xl animate-bounce font-bold font-mono">Zzz</div>
        </div>
      );
      case 'eating': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full relative">
           <div className={`w-24 h-24 ${cyanBase}`} />
      ...
      case 'rubik': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full relative">
           <div className={`w-20 h-20 ${cyanBase}`} />
           <div className="text-5xl animate-spin transition-all duration-1000">🎨</div>
           <div className={`w-20 h-20 ${cyanBase}`} />
        </div>
      );
      case 'mad': return (
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden" onMouseMove={resetIdleTimer} onTouchStart={resetIdleTimer}>
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        ...
        />
        <div className="w-full h-full flex items-center justify-center">{renderEyes()}</div>
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={() => setIsMuted(!isMuted)} className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1"><Volume2 size={18}/><span className="text-[8px] uppercase">Audio</span></button>
        </div>
      </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm mb-4 outline-none" />
            ...
            <button onClick={() => signOut(auth)} className="w-full mt-4 text-[10px] text-red-500 font-bold uppercase">Disconnect</button>
          </div>
        </div>
      )}
