...
export default function App() {
  ...
  const handleSend = async (manualText) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const name = getCurrentName();
        const system = `You are Eilo, an independent robot with blue eyes. Logan Baez is your creator. ${isLogan() ? "The user is Logan Baez!" : `The user is ${name}.`} Be sassy. Current view: ${cameraState}. NEVER use 😭 or 💀 emojis. Use 🎀, ✨, 🎈 instead.`;
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
