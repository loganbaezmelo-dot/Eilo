...
import { 
  ...
  Send, Battery, Wifi, Cpu, Zap, MessageSquare, Settings, X, Key, Mail, LogOut, AlertCircle, RefreshCcw
} from 'lucide-react';
...
export default function App() {
  ...
  // --- REPAIR MODE SCREEN 🛠️ ---
  if (isFirebaseMissing) {
    return (
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-8 font-sans text-center">
        <div className="w-20 h-20 bg-red-500/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
          <AlertCircle className="text-red-500" size={40} />
        </div>
        <h1 className="text-2xl font-bold mb-4">Eilo is in a Coma 💀</h1>
        <p className="text-slate-400 text-sm max-w-xs mb-8 leading-relaxed">
          I can't find his Firebase Soul. Make sure you added <code className="bg-white/10 px-2 py-1 rounded">VITE_FIREBASE_CONFIG</code> to your Vercel Environment Variables! 😭
        </p>
        <button onClick={() => window.location.reload()} className="flex items-center gap-2 bg-white/5 border border-white/10 px-6 py-3 rounded-2xl text-sm font-bold hover:bg-white/10 transition-all">
          <RefreshCcw size={16} /> Retry Booting
        </button>
      </div>
    );
  }

  // --- REST OF THE CODE (AUTH & UI) ---
        ...
        <div className="w-full max-w-sm bg-[#161622] rounded-[50px] p-10 border border-white/5 shadow-2xl text-center">
            ...
            {authView === 'choice' ? (
              <div className="space-y-4">
                <button onClick={handleGoogleLogin} className="w-full bg-white text-black py-4 rounded-2xl font-bold hover:bg-slate-200 transition-all active:scale-95">Google Sign In</button>
                <button onClick={() => setAuthView('email-login')} className="w-full bg-white/5 border border-white/10 py-4 rounded-2xl font-bold hover:bg-white/10 transition-all active:scale-95">Email & Password</button>
              </div>
            ) : (
              <div className="space-y-4 text-left">
                ...
                <button onClick={() => setAuthView(authView === 'email-login' ? 'email-signup' : 'email-login')} className="w-full text-center text-xs text-slate-500 hover:text-pink-400 transition-colors">
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[40px] p-8 border border-white/10 relative">
            ...
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2 text-pink-400">Brain Core ✨</h2>
            <div className="space-y-4">
               <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Gemini API Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-3 px-4 text-sm outline-none focus:border-pink-500/50" />
               <button onClick={() => { localStorage.setItem('eilo_key', tempApiKey); setShowSettings(false); }} className="w-full bg-pink-600 py-4 rounded-2xl font-bold text-sm active:scale-95 transition-all">Save Soul ✨</button>
            </div>
          </div>
        </div>
      )}
