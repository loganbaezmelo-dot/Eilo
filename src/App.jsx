...
export default function App() {
  ...
  // --- LANDSCAPE UI ARCHITECTURE (FIXED) ---
  if (isLandscape && !isChaosMode) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center overflow-hidden">
        ...
        {/* LAYER 1: FACE VISUALS (Bottom) */}
        <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none">
            {renderFace()}
        </div>

        {/* LAYER 2: PET SENSOR (Middle - catches empty space clicks) */}
        <div 
        ...
        {/* LAYER 3: UI OVERLAY (Top - Buttons Only) */}
        <div className="absolute inset-0 z-50 pointer-events-none p-8 flex flex-col justify-between">
            ...
            <div className="flex justify-end items-start pointer-events-auto">
                <div className="flex gap-4">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); toggleMic(); }} className={`p-6 rounded-full border transition-all ${isInfinityMic ? 'bg-cyan-500 border-cyan-400 animate-pulse' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10 hover:text-white'}`}>
                        <Mic size={32} />
                    </button>
                </div>
            </div>
            ...
            {/* Center: Agent Status */}
            {aiAgentMode && (
                <div className="absolute top-8 left-1/2 -translate-x-1/2 text-[10px] font-mono text-cyan-400 uppercase tracking-widest bg-cyan-950/40 px-6 py-2 rounded-full border border-cyan-500/20 z-50 pointer-events-none">
            ...
            <div className="flex justify-end items-end pointer-events-auto">
                <button onClick={(e) => { e.stopPropagation(); setShowSettings(true); }} className="p-6 rounded-full border border-white/10 bg-white/5 text-slate-500 hover:bg-white/10 hover:text-white transition-all">
                    <Settings size={32} />
                </button>
            </div>
        </div>
