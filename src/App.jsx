...
export default function App() {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        if (transcript.includes("hey eilo")) {
          const parts = transcript.split("hey eilo");
  ...
  const toggleInfinityMic = () => {
    ...
    if (nextState) {
      setIsListening(true);
      recognitionRef.current.start();
      speak("Infinity mic active. Say Hey Eilo to talk to me! ✨");
    } else {
      recognitionRef.current.stop();
      setIsListening(false);
      speak("Infinity mic off. 🧸");
    }
  };

  // --- UPDATED MOTION SENSORS (Sensitivity Fix) 📱 ---
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      // Higher threshold (28) so she isn't too sensitive 😭💀
      const threshold = 28; 
      const isShaking = Math.abs(acc.x) > threshold || Math.abs(acc.y) > threshold || Math.abs(acc.z) > threshold + 15;

      if (isShaking) {
        if (mood !== 'dizzy' && mood !== 'mad') {
          setMood('dizzy');
          speak("Whoa! Logan, stop shaking the phone! I'm so dizzy! 😭");
    ...
    const handleOrientation = (event) => {
      ...
      // Scared if tilted significantly
      if (Math.abs(event.beta) > 75 || Math.abs(event.gamma) > 45) {
        if (mood !== 'scared') {
          setMood('scared');
          speak("Eek! We're too high up! Don't drop me Logan! 😭💀");
        }
      } else if (mood === 'scared') {
        setMood('neutral');
        speak("Phew, much better. ✨");
      }
    };
  ...
  }, [isAwake, fearOfHeights, mood]);

  const enableSensors = async () => {
    if (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') {
      try { await DeviceMotionEvent.requestPermission(); await DeviceOrientationEvent.requestPermission(); } catch (e) { console.error("Sensor error 💀"); }
    }
  };

  const getLocalResponse = (text) => {
    const t = text.toLowerCase();
    const name = user?.displayName?.split(' ')[0] || "Owner";
    if (t.includes("who made you")) return "Logan Baez built me from scratch. He's the goat! ✨";
    if (t.includes("logan")) return "Logan? Yeah, he gave me these blue eyes. Pretty cool guy. 🧸";
    if (t.includes("hi") || t.includes("hello")) return `Hey ${name}! What's up? 🎀`;
    return "I'm just vibing in my digital world! 🧸";
  };
  ...
  const triggerIdleAction = () => {
    ...
    setMood(choice);
    if (choice === 'sleeping') { speak("Zzz. Sleeping mode. Zzz."); setTimeout(() => setMood('neutral'), 12000); }
    if (choice === 'eating') { speak("Nom nom! Virtual sandwich! ✨"); let bites = 0; const munch = setInterval(() => { if (bites > 8) { clearInterval(munch); setMood('neutral'); return; } playClick(); bites++; }, 1500); }
    if (choice === 'rubik') { speak("Cube time! 🧩"); let moves = 0; const solve = setInterval(() => { if (moves > 12) { clearInterval(solve); setMood('neutral'); return; } playClick(); moves++; }, 1000); }
  };
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        const firstName = u.displayName?.split(' ')[0] || "Owner";
        speak(`Yo ${firstName}! Eilo is online. 🎀`);
    ...
    try {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ contents: [{ parts: [{ text: userMsg }] }], systemInstruction: { parts: [{ text: `You are Eilo, a sweet, independent robot with blue eyes made by Logan Baez. Keep it cute. NO skulls/crying.` }] } })
            });
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex items-center justify-center gap-16 md:gap-32 h-full animate-[spin_0.5s_infinite_linear]"><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /></div>;
      ...
      case 'sleeping': return <div className="flex items-center justify-center gap-16 md:gap-32 h-full relative"><div className="w-24 h-3 bg-cyan-600 rounded-full" /><div className="w-24 h-3 bg-cyan-600 rounded-full" /><div className="absolute top-1/4 right-1/4 text-cyan-400 text-3xl animate-pulse font-mono font-bold">Zzz...</div></div>;
      case 'eating': return <div className="h-full flex flex-col items-center justify-center pt-8"><div className="flex gap-16 md:gap-32 mb-6"><div className={`w-24 h-24 ${cyanBase}`} /><div className={`w-24 h-24 ${cyanBase}`} /></div><div className="flex items-center gap-4"><div className="w-8 h-12 bg-cyan-600 rounded-full" /><div className="text-7xl animate-bounce">🥪</div><div className="w-8 h-12 bg-cyan-600 rounded-full" /></div></div>;
      case 'rubik': return <div className="h-full flex flex-col items-center justify-center pt-8"><div className="flex gap-16 md:gap-32 mb-6"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div><div className="flex items-center gap-6"><div className="w-8 h-12 bg-cyan-600 rounded-full" /><RubiksCube /><div className="w-8 h-12 bg-cyan-600 rounded-full" /></div></div>;
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden" onMouseMove={resetIdleTimer} onTouchStart={resetIdleTimer}>
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        <div className="flex-1 bg-[#161622] rounded-[40px] border border-white/5 p-6 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="mt-5 flex gap-2">
            <div className="relative flex-1">
                ...
                <button onClick={() => handleSend()} className="absolute right-2 top-2 p-3 bg-cyan-600 rounded-xl"><Send size={18}/></button>
            </div>
            <button onClick={toggleMic} className={`p-4 rounded-2xl transition-all border ${isListening ? 'bg-red-500 text-white border-red-400' : 'bg-white/5 text-slate-400 border-white/10'}`}>
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={handlePet} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95 transition-all"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
          ...
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={enableSensors} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all uppercase tracking-widest">
                    <Zap size={14}/> Calibrate Sensors
                  </button>
                  <button 
                    onClick={() => {
