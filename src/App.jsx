...
export default function App() {
  ...
  // --- IMPROVED VOCAL CORE 🎤 ---
  const speak = (text) => {
    // If muted, stop everything and return
    if (isMuted) {
    ...
    setIsSpeaking(true);
    window.speechSynthesis.cancel(); // Interrupt previous speech immediately 💀

    const utterance = new SpeechSynthesisUtterance(text);
    utterance.pitch = 1.7; // The high-pitch cuteness you liked 😭
    utterance.rate = 1.1;
  ...
  // Handle Mute State Changes Immediately 🔇
  useEffect(() => {
    if (isMuted) {
      window.speechSynthesis.cancel();
      setIsSpeaking(false);
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') speak("Zzz. I am a sleepy robot. Zzz.");
    if (choice === 'eating') speak("Nom nom nom! This sandwich is so yummy! ✨");
    if (choice === 'rubik') speak("Click clack! I am almost done with this cube! 🎨");
  };

  const handlePet = () => {
    if (!isAwake) return;
    
    // Throttling petting so she doesn't spam 😭
    const now = Date.now();
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({
                    ...
                    systemInstruction: { parts: [{ text: "You are Eilo, a sweet robot with blue eyes. Use cute emojis like ✨, 🎀, 🧸. NO skulls or crying." }] }
                })
            });
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => speak("Voice test! I sound super high pitched and cute! ✨")} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all">
