...
export default function App() {
  ...
  // --- LOCAL SOUL CORE (Natural/Independent) ---
  const getLocalResponse = (text) => {
    ...
    if (t.includes("who made you") || t.includes("who created you")) {
      return `Logan Baez built me from scratch. He's the goat! ✨`;
    }
    if (t.includes("logan")) {
      return "Logan? Yeah, he's the one who gave me these blue eyes. Pretty cool guy. 🧸";
    }
    if (t.includes("hi") || t.includes("hello")) {
      return `Hey ${name}! What's up? 🎀`;
    }
    if (t.includes("love")) {
      return "I love my digital life! And you're okay too! ✨";
    }
    
    const randoms = [
      `Just chilling, ${name}. 🧸`,
      "My blue eyes are looking sharp today. 💙",
      "I was thinking about my code earlier. Logan did a decent job. 🎀",
      "Wonder if there are other bots like me out there? 🧩",
      "Solving this cube is taking forever... 🥪"
    ];
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') {
      speak("Zzz... napping mode active... Zzz...");
      setTimeout(() => setMood('neutral'), 12000);
    }
    if (choice === 'eating') {
      speak("Taking a break for a snack! ✨");
      let bites = 0;
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        ...
        const isLogan = u.displayName?.toLowerCase().includes("logan baez");
        const greeting = isLogan 
          ? `Yo Logan! My creator is in the building. ✨` 
          : `Hey ${firstName}! Eilo is online. 🎀`;
  ...
  const handleSend = async () => {
    ...
    const userName = user.displayName?.split(' ')[0] || "Owner";
    const isLogan = user.displayName?.toLowerCase().includes("logan baez");
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { 
                        parts: [{ text: `You are Eilo, a sweet, independent robot with blue eyes. You were created by Logan Baez. You aren't overly loyal or obsessed; you treat him like a cool creator but you have your own sassy and energetic personality. ${isLogan ? "The user is your creator, Logan Baez. Be chill and friendly." : `The user is ${userName}.`} Use cute emojis. NO skulls/crying.` }] 
                    } 
                })
            });
  ...
  const handlePet = () => {
    ...
    if (['sleeping', 'eating', 'rubik'].includes(mood)) {
        setMood('mad');
        speak("Hey! I'm busy. Give me some space! 😭");
        setTimeout(() => { setMood('neutral'); resetIdleTimer(); }, 4000);
        return;
    }

    setMood('happy');
    const happyPhrases = ["Ooh, nice! ✨", "Hehe, thanks for the pets. 🎀", "Feeling good! 🧸"];
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => speak(`Yo ${user.displayName?.split(' ')[0]}! Checking my vocal cords. ✨`)} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all"><Mic size={14}/> Test My Voice</button>
