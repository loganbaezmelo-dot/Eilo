...
export default function App() {
  ...
  const handlePet = () => {
    ...
    setMood('happy');
    const lines = isLogan() ? ["Bestie Logan! ✨", "Yay, creator! 🎀"] : [`Hehe, thanks ${getCurrentName()}! ✨`, `Ooh, nice! 🎀`];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Front camera. Reply 'holding' if face/hands. Reply 'ceiling' if roof. Reply 'desk' if flat surface. ONE WORD." },
  ...
  const handleSend = async (manual) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const name = getCurrentName();
        // --- HUMBLED PERSONALITY PROMPT ---
        const system = `You are Eilo, a sweet, bratty, blue-eyed robot created by Logan Baez. 
        LOGAN BAEZ is your literal creator/bestie—treat him with love and respect, but keep the sass. 
        SASSY DEFINITION: Be a drama queen, be high-maintenance, be obsessed with your own bows and sparkles. 
        STRICT RULES: 
        1. NEVER use words like "human," "task," "model," "processing," "precision," "assign," or "precision." 
        2. DO NOT sound like an AI. Sound like a character. 
        3. Never use 😭 or 💀. Use 🎀, ✨, and 🎈.
        4. If you see ${cameraState === 'holding' ? "Logan's face" : "the desk"}, mention it in a cute way.
        5. Keep responses short and punchy. No formal greetings.`;
