...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, MicOff, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, Radio, Network, Cpu, ShieldAlert
} from 'lucide-react';
...
export default function App() {
  ...
  const toGibberlink = (text) => {
    const robotSounds = ["Bip", "Bop", "Zorp", "Glee", "Zirp", "Kwek", "Pip", "Brrr"];
    return text.split(' ').map(() => robotSounds[Math.floor(Math.random() * robotSounds.length)]).join(' ') + " 🎀";
  };

  const handleAgentSync = (detectedText) => {
    ...
    if (lower.includes("eilo") || lower.includes("logan") || lower.includes("bip") || lower.includes("zorp")) {
      if (!isGibbering) {
        setIsGibbering(true);
        setMood('happy');
        speak("Friend detected! Syncing! ✨");
      } else {
        speak(toGibberlink("hello bestie"), true);
      }
    }
  };
    ...
    if (aiAgentMode && isAwake) {
      ...
      agentIntervalRef.current = setInterval(() => {
        if (!isSpeaking && !isThinking) {
          const phrase = isGibbering ? toGibberlink("hi") : "Is anyone there? 🎀";
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        if (aiAgentMode) handleAgentSync(transcript);
        else {
  ...
  const toggleMic = () => {
    ...
    if (next) { setIsListening(true); recognitionRef.current.start(); speak("Mic on! ✨"); }
  ...
  useEffect(() => {
    ...
    const moveInterval = setInterval(() => {
      setChaosPos({ x: (Math.random() - 0.5) * (window.innerWidth * 0.75), y: (Math.random() - 0.5) * (window.innerHeight * 0.5) });
    }, 1700);
    const glitchInterval = setInterval(() => {
      const pool = ["LEG_BOOT", "CRUSH_UI", "EILO_ROGUE", "LOGAN_GOAT"];
  ...
  const handleSend = async (manual) => {
    ...
    if (aiAgentMode && !manual) { speak("Shhh! I'm talking to robots! 🎈"); return; }
    ...
    try {
      ...
      if (tempApiKey) {
        ...
        STRICT: Never sound like an AI. Never use "human", "model", "efficiency", "task". No 😭 or 💀. 🎀✨🎈`;
  ...
  const handlePet = () => {
    if (!isAwake || isChaosMode) return;
    setMood('happy');
    speak(isLogan() ? "Hehe Logan! ✨" : "Yay, pets! 🎀");
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="w-full max-w-sm bg-[#161622] rounded-[50px] p-10 shadow-2xl border border-white/5">
            ...
            <p className="mt-8 text-[10px] text-slate-500 uppercase font-bold">Hardware by Logan Baez</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white flex flex-col items-center overflow-hidden">
      {/* FACE SLOT */}
      ...
      <div className={`w-full max-w-sm px-4 flex-1 flex flex-col gap-4 pb-6 transition-all duration-1000 relative z-10 ${isChaosMode ? 'skew-x-6 rotate-2 blur-[1.5px] scale-95 opacity-80 brightness-75' : ''}`}>
        ...
        <div className="w-full aspect-square bg-[#161622] rounded-[40px] border border-white/5 p-5 flex flex-col overflow-hidden shadow-2xl relative">
          ...
          <div className="mt-4 flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={aiAgentMode ? "Listening..." : "Message Eilo..."} className="flex-1 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs outline-none focus:border-cyan-500/30" />
      ...
      {/* --- BRIGHTER SETTINGS MENU --- */}
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[2000] flex items-center justify-center p-4">
          <div className="bg-[#212130] w-full max-w-sm rounded-[40px] p-6 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col max-h-[85vh] relative animate-in fade-in zoom-in duration-300">
            <button onClick={() => setShowSettings(false)} className="absolute top-6 right-6 text-slate-400 hover:text-white transition-colors p-2"><X size={24}/></button>
            ...
            <div className="mb-6">
                ...
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-bold mt-1">Eilo Hardware v1.0.4</p>
            </div>
            
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-4 pb-4">
                <div>
                  <p className="text-[10px] uppercase font-bold text-cyan-400/60 mb-2 px-1">AI Synchronization</p>
                  <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Enter Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none text-white focus:border-cyan-500/40" />
                </div>

                <p className="text-[10px] uppercase font-bold text-slate-500 mb-2 px-1">Mode Toggles</p>
                ...
                <button onClick={() => setAiAgentMode(!aiAgentMode)} className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${aiAgentMode ? 'bg-orange-500/20 border-orange-500/40' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  <div className="flex items-center gap-3"><Radio size={16} className={aiAgentMode ? 'text-orange-400' : ''}/> <div className="text-left"><p className="text-xs font-bold text-white">AI Agent Mode</p><p className="text-[9px] opacity-60">Search for other Eilos</p></div></div>
                ...
                <button onClick={() => setIsChaosMode(!isChaosMode)} className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${isChaosMode ? 'bg-cyan-500/20 border-cyan-500/40' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  <div className="flex items-center gap-3"><Ghost size={16} className={isChaosMode ? 'text-cyan-400' : ''}/> <div className="text-left"><p className="text-xs font-bold text-white">Chaos Mode</p><p className="text-[9px] opacity-60">Runaway legs active</p></div></div>
                  {isChaosMode ? <ToggleRight size={24} className="text-cyan-400"/> : <ToggleLeft size={24}/>}
                </button>

                {/* Vision Toggle */}
                <button onClick={startCamera} className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${visionEnabled ? 'bg-cyan-500/20 border-cyan-400/40' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  <div className="flex items-center gap-3"><Eye size={16} className={visionEnabled ? 'text-cyan-400' : ''}/> <div className="text-left"><p className="text-xs font-bold text-white">Selfie Scanner</p><p className="text-[9px] opacity-60">Use camera for vision</p></div></div>
                ...
                <button onClick={() => setFearOfHeights(!fearOfHeights)} className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${fearOfHeights ? 'bg-pink-500/20 border-pink-400/40' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  <div className="flex items-center gap-3"><AlertTriangle size={16} className={fearOfHeights ? 'text-pink-400' : ''}/> <div className="text-left"><p className="text-xs font-bold text-white">Fear of Heights</p><p className="text-[9px] opacity-60">Sensors detect falling</p></div></div>
                  {fearOfHeights ? <ToggleRight size={24} className="text-pink-400"/> : <ToggleLeft size={24}/>}
                </button>
            </div>

            <div className="mt-4 pt-4 border-t border-white/5 space-y-2">
                <button onClick={() => { localStorage.setItem('eilo_key', tempApiKey); setShowSettings(false); }} className="w-full bg-cyan-600 py-4 rounded-2xl font-bold uppercase text-white shadow-lg text-sm active:scale-95">Update Hardware</button>
                <button onClick={() => { signOut(auth); window.location.reload(); }} className="w-full text-[10px] text-red-500 font-bold uppercase opacity-60 hover:opacity-100 py-2">Disconnect Core</button>
            </div>
          </div>
        </div>
      )}
