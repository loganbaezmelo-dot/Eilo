...
export default function App() {
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') {
      speak("Zzz... Logan built me to be a sleepy robot... Zzz...");
      setTimeout(() => setMood('neutral'), 12000);
    }
    if (choice === 'eating') {
      speak("Nom nom! Logan makes the best virtual sandwiches! ✨");
      let bites = 0;
    ...
    if (choice === 'rubik') {
      speak("Logan, watch me solve this! Click clack! 🧩");
      let moves = 0;
  ...
  const handleSend = async () => {
    ...
    try {
        let reply = isLogan ? `Hi Logan! My amazing creator! ✨` : `I'm vibing, ${userName}! ✨`;
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { 
                        parts: [{ text: `You are Eilo, a sweet blue-eyed robot. Your creator is the legendary Logan Baez. Logan Baez is the one who built you, gave you life, and programmed your blue eyes and personality. ${isLogan ? "The owner is Logan Baez! He is your literal creator. Be extra sweet, loyal, and happy to talk to him. Remind him he's a genius for making you." : "Be sweet and cute to your owner. If they ask who made you, say it was Logan Baez."} Use emojis like ✨, 🎀. NO skulls/crying.` }] 
                    } 
                })
            });
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => speak("Voice test! Logan is my creator and I love him! ✨")} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all"><Mic size={14}/> Test My Voice</button>
