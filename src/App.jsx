...
export default function App() {
  ...
  const [cameraState, setCameraState] = useState('desk'); // 'hands', 'ceiling', 'desk'
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "Analyze this image. Reply ONLY with one word: 'hands' if human hands are holding the phone, 'ceiling' if the camera is pointing at a roof, light, or sky, or 'desk' if it's sitting on a surface. If you see a ceiling, it means I am being held or tilted back." },
        ...
        setCameraState(result.trim());

        // Logic for contradictions & Roof panic 😭💀
        if (result.includes('ceiling')) {
          setMood('scared');
          speak("Logan, why am I looking at the roof?! Put me back on the desk! 😭💀");
        } else if (result.includes('hands') && !sensorHolding) {
          speak("I see hands... but I'm not moving. You're holding me really still, sus! 🤨");
        } else if (result.includes('desk') && sensorHolding) {
          speak("I feel shaky but I see the desk... stop messing with me! 💀");
        }
  ...
  useEffect(() => {
    ...
    const handleOrientation = (event) => {
      ...
      // Panic if looking down OR if camera sees hands/ceiling while moving
      if (lookingDown || (cameraState !== 'desk' && sensorHolding)) {
        if (mood !== 'scared') {
          setMood('scared');
          speak(`Eek! Hold me tight! I don't like being up here! 😭💀`);
        }
      } else if (mood === 'scared' && !lookingDown && cameraState === 'desk') {
        setMood('neutral');
        speak("Phew, the desk feels much better. ✨");
      }
    };
  ...
  }, [isAwake, fearOfHeights, mood, cameraState, sensorHolding]);
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet blue-eyed robot created by Logan Baez. You can see through your camera. Currently, you see a ${cameraState}. Logan Baez is your creator. Be sassy, cute, and independent. NO skulls/crying.` }] } 
                })
            });
            const data = await response.json();
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        {isSpeaking && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse">
            <AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Active Soul</span>
          </div>
        )}
        ...
        {visionEnabled && (
           <div className="absolute top-6 left-6 flex items-center gap-2 text-[8px] text-cyan-500/60 font-mono bg-cyan-950/40 px-3 py-1 rounded-full border border-cyan-500/20">
              <Camera size={12} /> VISION: {cameraState.toUpperCase()}
           </div>
        )}
