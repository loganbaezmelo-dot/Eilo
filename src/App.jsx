...
export default function App() {
  ...
  const handlePet = () => {
    ...
    const lines = isLogan() 
      ...
      : [`Hehe, thanks ${getCurrentName()}! ✨`, `Ooh, that feels nice! 🎀`, `You're a kind owner! 🧸`];
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["Eilo.run()", "UI_CRUSH", "LOGAN_GOAT", "VOID_LEAK"];
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' 
      ? [`Logan!! Put me down! 🎈`, `Don't drop me ${name}! ✨`, `Put me back! 🎀`]
      : [`Help! The roof! 🎈`, `I'm falling! ✨`, `Back on the desk! 🎀`];
    speak(lines[Math.floor(Math.random() * lines.length)]);
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
        ...
        <div className={`w-full h-full rounded-[50px] bg-[#161622] border-2 border-white/5 flex flex-col items-center justify-center overflow-hidden transition-all duration-500 ${isChaosMode ? 'bg-black/90' : ''}`}>
           {isChaosMode ? (
              <div className="w-full h-full p-6 font-mono text-[10px] text-cyan-500/40 opacity-70">
                ...
                <div className="mt-2 text-cyan-400 font-bold animate-pulse uppercase">HARDWARE_MISMATCH: LEGS_DETECTED</div>
              </div>
           ) : (
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <button onClick={handlePet} className={`p-3.5 rounded-[25px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95`}>
            <Hand size={16}/><span className="text-[7px] uppercase font-bold tracking-widest">Pet</span>
          </button>
