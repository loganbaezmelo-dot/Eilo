...
export default function App() {
  ...
  // --- VISION CORE (Smarter Selfie Eyes) 👁️ ---
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "You are a robot's vision processor. Look at this selfie-camera photo. If you see a human face, hands, or a person, respond 'holding'. If you see a ceiling, ceiling fan, lights, or indoor roof, respond 'ceiling'. If you see a flat stationary surface like a wooden desk, table, or floor with NO people in view, respond 'desk'. Be extremely literal. Reply with ONLY one word." },
        ...
        // Updated logic: Face/Hands or Ceiling triggers fear 😭💀
        if (result === 'ceiling' || result === 'holding') {
          setMood('scared');
          if (result === 'holding') {
            speak(`I see you ${name}! Stop holding me, I'm scared! 😭💀`);
          } else {
            speak(`Why am I looking at the ceiling ${name}?! Put me down! 😭`);
          }
        }
  ...
  useEffect(() => {
    if (visionEnabled) {
      visionTimerRef.current = setInterval(analyzeEnvironment, 6000); // Slightly faster checks
    }
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet robot with blue eyes. Logan Baez is your creator. Your current view is ${cameraState}. Be sassy and independent. NO skulls/crying.` }] } 
                })
            });
            const data = await response.json();
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        speak(`Yo ${u.displayName?.split(' ')[0]}! Eilo's here! 🎀`);
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="w-full max-w-sm bg-[#161622] border border-white/5 rounded-[50px] p-10 shadow-2xl font-sans">
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        {isSpeaking && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse">
            <AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Visual Soul</span>
          </div>
        )}
