...
import { 
  ...
  Send, Battery, Wifi, Zap, Settings, X, LogOut, Hand, AudioLines, Mic, ToggleLeft, ToggleRight, AlertTriangle
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      // Shake detection
      const threshold = 15;
      if (Math.abs(acc.x) > threshold || Math.abs(acc.y) > threshold || Math.abs(acc.z) > threshold + 10) {
        if (mood !== 'dizzy') {
          setMood('dizzy');
          speak("Whoa! Too much shaking! I'm dizzy! 😭");
    ...
    const handleOrientation = (event) => {
      if (!isAwake || !fearOfHeights || mood === 'dizzy') return;
      
      // Fear of heights detection (if tilted forward/backward significantly)
      // Beta is tilt front-to-back. If phone is held upright or tilted down (looking at ground)
      if (Math.abs(event.beta) > 70 || Math.abs(event.gamma) > 40) {
        if (mood !== 'scared') {
          setMood('scared');
  ...
  const triggerIdleAction = () => {
    ...
    setMood(choice);
    if (choice === 'sleeping') { speak("Zzz... napping mode active... Zzz..."); setTimeout(() => setMood('neutral'), 12000); }
    if (choice === 'eating') { speak("Taking a break for a snack! ✨"); let bites = 0; const munch = setInterval(() => { if (bites > 8) { clearInterval(munch); setMood('neutral'); return; } playClick(); bites++; }, 1500); }
    if (choice === 'rubik') { speak("Cube time! Watch me work. 🧩"); let moves = 0; const solve = setInterval(() => { if (moves > 12) { clearInterval(solve); setMood('neutral'); return; } playClick(); moves++; }, 1000); }
  };
    ...
    try {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ contents: [{ parts: [{ text: userMsg }] }], systemInstruction: { parts: [{ text: `You are Eilo, an independent robot with blue eyes made by Logan Baez. You are chill and sassy. NO skulls/crying.` }] } })
            });
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full animate-[spin_1s_infinite_linear]">
          <div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full" />
          <div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full" />
        </div>
      );
      case 'scared': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full animate-bounce">
          <div className="w-20 h-12 bg-cyan-100 rounded-full shadow-[0_0_20px_white]" />
          <div className="w-20 h-12 bg-cyan-100 rounded-full shadow-[0_0_20px_white]" />
        </div>
      );
      case 'thinking': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full">
          <div className="w-20 h-20 bg-blue-300 rounded-full animate-pulse" /><div className="w-20 h-20 bg-blue-300 rounded-full animate-pulse" />
        </div>
      );
      case 'happy': return (
      ...
      case 'eating': return (
        <div className="h-full flex flex-col items-center justify-center pt-8">
           <div className="flex gap-16 md:gap-32 mb-6"><div className={`w-24 h-24 ${cyanBase}`} /><div className={`w-24 h-24 ${cyanBase}`} /></div>
           <div className="flex items-center gap-4"><div className="w-8 h-12 bg-cyan-600 rounded-full" /><div className="text-7xl animate-bounce">🥪</div><div className="w-8 h-12 bg-cyan-600 rounded-full" /></div>
        </div>
      );
      case 'rubik': return (
        <div className="h-full flex flex-col items-center justify-center pt-8">
           <div className="flex gap-16 md:gap-32 mb-6"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div>
           <div className="flex items-center gap-6"><div className="w-8 h-12 bg-cyan-600 rounded-full" /><RubiksCube /><div className="w-8 h-12 bg-cyan-600 rounded-full" /></div>
        </div>
      );
      case 'mad': return (
        <div className="flex items-center justify-center gap-16 md:gap-32 h-full">
