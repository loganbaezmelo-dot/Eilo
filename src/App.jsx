...
export default function App() {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({
                    ...
                    systemInstruction: { parts: [{ text: "You are Eilo, a sweet, energetic robot with cute blue eyes. You love your owner. Use emojis like ✨, 🎀, 🧸. NEVER use skulls or crying emojis." }] }
                })
            });
            const data = await response.json();
  ...
  const renderEyes = () => {
    const eyeBase = "bg-cyan-400 rounded-3xl animate-[blink_4s_infinite] shadow-[0_0_35px_rgba(34,211,238,0.7)]";
    if (!isAwake) return <div className="text-cyan-900/10"><Moon size={48}/></div>;
    if (mood === 'thinking') return <div className="flex gap-16 md:gap-32"><div className="w-20 h-20 bg-blue-300 rounded-full animate-pulse blur-[1px]" /><div className="w-20 h-20 bg-blue-300 rounded-full animate-pulse blur-[1px]" /></div>;
    if (mood === 'happy') return <div className="flex gap-16 md:gap-32"><div className="w-24 h-16 bg-cyan-400 rounded-full animate-bounce flex items-center justify-center"><div className="w-8 h-8 bg-white/30 rounded-full" /></div><div className="w-24 h-16 bg-cyan-400 rounded-full animate-bounce flex items-center justify-center"><div className="w-8 h-8 bg-white/30 rounded-full" /></div></div>;
    return <div className="flex gap-16 md:gap-32"><div className={`w-24 h-24 ${eyeBase}`} /><div className={`w-24 h-24 ${eyeBase}`} /></div>;
  };
      ...
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-6 font-sans text-center">
        <div className="w-full max-w-sm bg-[#161622] border border-white/5 rounded-[50px] p-10 shadow-2xl">
            ...
            <div className="space-y-4">
                <button onClick={() => signInWithPopup(auth, googleProvider)} className="w-full bg-white text-black py-4 rounded-2xl font-bold active:scale-95 transition-all">Login to Sync ✨</button>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Connect to wake up Eilo</p>
            </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white flex flex-col items-center justify-center overflow-hidden">
      <div className="relative portrait:w-[92%] portrait:h-64 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black">
        <div className="absolute top-8 px-12 w-full flex justify-between items-center opacity-30 text-[10px] portrait:flex landscape:hidden font-mono tracking-widest uppercase">
          <div className="flex items-center gap-2 text-cyan-400"><Heart size={10} fill="currentColor"/> {user.email?.split('@')[0]}'s Eilo</div>
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        <div className="flex-1 bg-[#161622] rounded-[40px] border border-white/5 p-6 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="mt-5 relative">
            ...
            <button id="send-btn" onClick={handleSend} className="absolute right-2 top-2 p-3 bg-cyan-600 rounded-xl shadow-lg active:scale-95 transition-all"><Send size={18}/></button>
          </div>
        </div>
        
        <div className="grid grid-cols-2 gap-4 pb-4">
          <button onClick={() => setIsAwake(!isAwake)} className={`p-5 rounded-[35px] border border-white/5 flex flex-col items-center gap-1 transition-all ${isAwake ? 'bg-white/5 text-cyan-200' : 'bg-indigo-500/5 text-indigo-400 border-indigo-500/10'}`}>
          ...
          <button onClick={() => setIsMuted(!isMuted)} className={`p-5 rounded-[35px] border border-white/5 flex flex-col items-center gap-1 transition-all ${isMuted ? 'text-red-400 bg-red-400/5' : 'bg-white/5 text-cyan-200'}`}>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="w-16 h-16 bg-cyan-500/10 rounded-3xl flex items-center justify-center mx-auto mb-6"><Sparkles className="text-cyan-400" size={32}/></div>
            <h2 className="text-xl font-bold mb-2">Eilo Heart ✨</h2>
            <p className="text-xs text-slate-400 mb-8 leading-relaxed px-4">Paste your Gemini key below to hear Eilo speak! 🧸🎀</p>
            <div className="space-y-4">
               ...
               <button onClick={() => { localStorage.setItem('eilo_key', tempApiKey); setShowSettings(false); }} className="w-full bg-cyan-600 py-4 rounded-2xl font-bold shadow-lg shadow-cyan-900/20 active:scale-95 transition-all uppercase tracking-widest">Update Brain Core</button>
