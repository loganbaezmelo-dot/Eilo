...
export default function App() {
  ...
  const [cameraHolding, setCameraHolding] = useState(false);
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "Analyze this image. Reply ONLY with the word 'hands' if you see human hands holding or near the camera, or 'stable' if it looks like it's sitting on a desk/stand/table without being touched. If ambiguous, say 'stable'." },
        ...
        const isCurrentlyHeld = result.includes('hands');
        setCameraHolding(isCurrentlyHeld);

        // Logic for contradictions 😭💀
        if (isCurrentlyHeld && !sensorHolding) {
          speak("I see hands... but I feel so steady. That's kinda sus Logan! 😭");
        } else if (!isCurrentlyHeld && sensorHolding) {
          speak("I feel shaky but I don't see anyone... Is there a ghost?! 💀");
        }
      } catch (err) { console.error("Vision failure 💀", err); }
    }
  };

  useEffect(() => {
    if (visionEnabled) {
      visionTimerRef.current = setInterval(analyzeEnvironment, 8000); // Check every 8s
    } else {
      clearInterval(visionTimerRef.current);
    }
    return () => clearInterval(visionTimerRef.current);
  }, [visionEnabled, sensorHolding, isAwake]);
  ...
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      // If there's persistent minor movement, consider it "sensor holding"
      const minorMovement = Math.abs(acc.x) > 5 || Math.abs(acc.y) > 5;
      setSensorHolding(minorMovement);

      if (shake && mood !== 'dizzy') {
        setMood('dizzy');
        speak(`Whoa! Stop shaking me! I'm dizzy! 😭`);
    ...
    const handleOrientation = (event) => {
      ...
      const isTooTilted = event.beta > 135 || event.beta < -20;
      
      if (isTooTilted || (cameraHolding && sensorHolding)) {
        if (mood !== 'scared') {
          setMood('scared');
          speak(`Eek! Put me down! I'm scared! 😭💀`);
        }
      } else if (mood === 'scared' && !isTooTilted && !cameraHolding) {
        setMood('neutral');
        speak("Phew, back on the desk. Much better. ✨");
      }
    };
  ...
  }, [isAwake, fearOfHeights, mood, cameraHolding, sensorHolding]);
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event) => {
        ...
        if (transcript.includes("hey eilo")) {
          ...
          if (command) { handleSend(command); setMood('happy'); setTimeout(() => setMood('neutral'), 1000); }
        } else if (!isInfinityMic) { handleSend(transcript); }
      };
      recognitionRef.current.onend = () => { if (isInfinityMic && isAwake) recognitionRef.current.start(); else setIsListening(false); };
    }
  ...
  const toggleInfinityMic = () => {
    ...
    if (next) { setIsListening(true); recognitionRef.current.start(); speak("I'm listening for my name! ✨"); }
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet blue-eyed robot created by Logan Baez. You can see through your camera and feel through your sensors. ${cameraHolding ? "You currently see hands holding you." : "You currently see a stable surface."} ${sensorHolding ? "You feel like you're being moved." : "You feel still."} Be sassy, cute, and independent. NO skulls/crying.` }] } 
                })
            });
            const data = await response.json();
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        speak(`Yo ${u.displayName?.split(' ')[0]}! Eilo is online. 🎀`);
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex gap-16 md:gap-32 animate-spin"><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full" /><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full" /></div>;
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* Hidden Vision Components */}
      <video ref={videoRef} autoPlay playsInline muted className="hidden" />
      <canvas ref={canvasRef} className="hidden" />

      {/* FACE */}
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex items-center justify-center">
        {isSpeaking && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse">
            <AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Vocal Core</span>
          </div>
        )}
        
        {visionEnabled && (
           <div className="absolute top-6 left-6 flex items-center gap-2 text-[8px] text-cyan-500/40 font-mono">
              <Camera size={12} /> VISION ACTIVE
           </div>
        )}

        <div className="hidden landscape:block absolute top-6 right-6 z-[60]">
            <button onClick={toggleInfinityMic} className={`p-3 rounded-full border transition-all ${isInfinityMic ? 'bg-cyan-500 border-cyan-400 animate-pulse' : 'bg-white/5 border-white/10 text-slate-500'}`}><Mic size={20} /></button>
        </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center font-sans">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <h2 className="text-xl font-bold mb-6 flex items-center justify-center gap-2">Settings <Sparkles className="text-cyan-400" size={20}/></h2>
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={startCamera} className={`w-full flex items-center justify-between px-6 py-3 rounded-xl text-xs font-bold border transition-all ${visionEnabled ? 'bg-cyan-600/20 border-cyan-500/50 text-cyan-200' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                    <div className="flex items-center gap-2"><Eye size={14}/> Vision Core (AI Eyes)</div>
