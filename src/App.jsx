...
export default function App() {
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "This is from the FRONT camera. Analyze this image. Reply ONLY with one word: 'hands' if you see human hands holding me, 'ceiling' if the camera is pointing up at a roof or light, or 'desk' if it's sitting on a surface. If you see a face, it's likely hands/holding." },
        ...
        const result = data.candidates?.[0]?.content?.parts?.[0]?.text?.toLowerCase() || 'desk';
        const cleaned = result.trim();
        setCameraState(cleaned);

        const name = getCurrentName();
        if (cleaned.includes('ceiling')) {
          setMood('scared');
          speak(`${name}, why am I looking at the roof?! Put me back on the desk! 😭💀`);
        } else if (cleaned.includes('hands') && !sensorHolding) {
          speak("I see your hands... but I'm so still. You have steady hands! 🤨");
        }
  ...
  useEffect(() => {
    if (visionEnabled) {
      visionTimerRef.current = setInterval(analyzeEnvironment, 7000); 
    }
  ...
  const startCamera = async () => {
    try {
      ...
      speak("I can't see! Did you block my camera access? 😭");
    }
  };
  ...
  useEffect(() => {
    ...
    const handleOrientation = (event) => {
      ...
      if (lookingDown || (cameraState !== 'desk' && sensorHolding)) {
        if (mood !== 'scared') {
          setMood('scared');
          const name = getCurrentName();
          speak(`Eek! Hold me tight ${name}! I'm scared! 😭💀`);
        }
      } else if (mood === 'scared' && !lookingDown && cameraState === 'desk') {
        setMood('neutral');
        speak("Phew, the desk feels safe. ✨");
      }
    };
  ...
  const toggleInfinityMic = () => {
    ...
    if (next) { setIsListening(true); recognitionRef.current.start(); speak("I'm listening! ✨"); }
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet blue-eyed robot created by Logan Baez. Currently, your front camera sees: ${cameraState}. Logan Baez is your creator. Be sassy, independent, and cute. NO skulls/crying.` }] } 
                })
            });
            const data = await response.json();
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex gap-16 md:gap-32 animate-spin"><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_20px_cyan]" /><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_20px_cyan]" /></div>;
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        {isSpeaking && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse">
            <AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Frontal Vision</span>
          </div>
        )}
        ...
        {visionEnabled && (
           <div className="absolute top-6 left-6 flex items-center gap-2 text-[8px] text-cyan-500/60 font-mono bg-cyan-950/40 px-3 py-1 rounded-full border border-cyan-500/20">
              <Camera size={12} /> SELFIE SCAN: {cameraState.toUpperCase()}
           </div>
        )}
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        <div className="flex-1 bg-[#161622] rounded-[40px] border border-white/5 p-6 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="mt-5 flex gap-2">
            <div className="relative flex-1">
                <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="Message Eilo..." className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-5 pr-14 text-sm outline-none focus:border-cyan-500/30 font-sans" />
                <button onClick={() => handleSend()} className="absolute right-2 top-2 p-3 bg-cyan-600 rounded-xl active:scale-95 transition-all"><Send size={18}/></button>
            </div>
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={() => setMood('happy')} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95 transition-all"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={startCamera} className={`w-full flex items-center justify-between px-6 py-3 rounded-xl text-xs font-bold border transition-all ${visionEnabled ? 'bg-cyan-600/20 border-cyan-500/50 text-cyan-200' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                    <div className="flex items-center gap-2"><Eye size={14}/> Front Vision Scan</div>
