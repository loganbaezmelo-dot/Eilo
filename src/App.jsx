...
import { 
  ...
  Send, Battery, Wifi, Cpu, Zap, MessageSquare, Settings, X, Key, LogOut, Hand, Coffee, AudioLines
} from 'lucide-react';
...
export default function App() {
  ...
  const speak = async (text) => {
    ...
    try {
      ...
      if (pcmData) {
        ...
        writeString(0, 'RIFF'); view.setUint32(4, 36 + pcmBuffer.length, true); writeString(8, 'WAVE'); writeString(12, 'fmt ');
        view.setUint32(16, 16, true); view.setUint16(20, 1, true); view.setUint16(22, 1, true);
        view.setUint32(24, 24000, true); view.setUint32(28, 24000 * 2, true); view.setUint16(32, 2, true);
    ...
    } catch (err) { console.error("Vocal failure 💀:", err); setIsSpeaking(false); }
  };
  ...
  const handlePet = () => {
    ...
    setMood('happy');
    speak(["That feels so nice! ✨", "Hehe, I love pets! 🎀", "You're so kind! 🧸"][Math.floor(Math.random() * 3)]);
    ...
    try {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ contents: [{ parts: [{ text: userMsg }] }], systemInstruction: { parts: [{ text: "You are Eilo, a sweet, energetic robot with blue eyes. Use cute emojis like ✨, 🎀, 🧸. NO skulls or crying." }] } })
            });
            const data = await response.json();
    ...
    switch (mood) {
      ...
      case 'eating': return (
        <div className="h-full flex flex-col items-center justify-center pt-8">
           ...
           <div className="flex items-center gap-4">
             <div className="w-8 h-12 bg-cyan-600 rounded-full -rotate-12 shadow-lg" />
      ...
      case 'rubik': return (
        <div className="h-full flex flex-col items-center justify-center pt-8">
           ...
           <div className="flex items-center gap-4">
             <div className="w-8 h-12 bg-cyan-600 rounded-full -rotate-12 shadow-lg" />
             <div className="text-6xl animate-spin transition-all duration-1000 z-10">🎨</div>
             <div className="w-8 h-12 bg-cyan-600 rounded-full rotate-12 shadow-lg" />
           </div>
        </div>
      );
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        {isSpeaking && (
          <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse">
            <AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Transmitting...</span>
          </div>
        )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm mb-4 outline-none text-center" />
