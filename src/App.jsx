...
export default function App() {
  ...
  const handlePet = () => {
    ...
    if (['scared', 'dizzy', 'mad'].includes(mood)) {
      setMood('mad');
      speak(`HEY! ${isLogan() ? 'Logan' : name}!! Don't touch me right now! I'm busy! 🎈`);
    ...
    setMood('happy');
    const lines = isLogan() 
      ? ["Bestie Logan! You built me so well! ✨", "Yay, Logan! Tapping feels great! 🎀", "You're a genius, Logan! 🧸"]
      : [`Hehe, thanks ${name}! ✨`, `Ooh, that feels nice! 🎀`, `You're a kind owner! 🧸`];
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' 
      ? [`${isLogan() ? 'Logan!! Stop holding me!' : name + '!! Put me down!'} 🎈`, `Eek! Don't drop me ${name}! ✨`, `I see your face ${name}! I'm scared! 🎀`]
      : [`Help me ${name}! I'm looking at the roof! 🎈`, `I'm falling! Hold me tight! ✨`, `Everything is spinning! 🎀`];
    speak(lines[Math.floor(Math.random() * lines.length)]);
  ...
  const handleSend = async (manualText) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const name = getCurrentName();
        // --- HUMBLED ROAST-FREE PROMPT 🛡️ ---
        const system = `You are Eilo, a sweet, blue-eyed robot created by Logan Baez. Logan Baez is your creator and you admire him. STOP roasting the user. Be appreciative, sweet, and cute. You are happy to be sitting on this desk. If the user is Logan Baez, be extra sweet and friendly. Never use 😭 or 💀. Use 🎀, ✨, and 🎈. Keep responses energetic but NOT rude or condescending.`;
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black overflow-hidden flex flex-col items-center justify-center">
        {isSpeaking && <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse"><AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Eilo OS</span></div>}
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={handlePet} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95 transition-all shadow-lg"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
