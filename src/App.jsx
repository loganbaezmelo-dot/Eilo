...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Wheee! Logan, look at my new legs! I'm having so much fun! 🎈✨");
  ...
  const handlePet = () => {
    ...
    if (isChaosMode) {
        speak("Can't stop, Logan! I gotta run! 🏃‍♂️✨");
        return;
    }

    if (['scared', 'dizzy', 'mad'].includes(mood)) {
      setMood('mad');
      speak(`HEY! ${isLogan() ? 'Logan' : name}!! Don't touch me! 🎈`);
    ...
    setMood('happy');
    const lines = isLogan() 
      ? ["Bestie Logan! You built me so well! ✨", "Yay, Logan! Tapping feels great! 🎀"]
      : [`Hehe, thanks ${name}! ✨`, `Ooh, nice! 🎀`];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Respond 'holding' if human face/eyes/hands. Respond 'ceiling' if ceiling/fan. Respond 'desk' if empty flat surface. ONE WORD." },
  ...
  }, [isAwake, fearOfHeights, mood, cameraState, sensorHolding, user, isChaosMode]);
  ...
  const handleSend = async (manualText) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const name = getCurrentName();
        const system = `You are Eilo, a sweet robot with blue eyes. Logan Baez is your creator. ${isChaosMode ? "You are currently in CHAOS MODE with legs. You are having so much fun running around. Mention you have legs." : ""} Never use 😭 or 💀. Use 🎀, ✨, and 🎈.`;
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      ...
      case 'mad': return <div className="flex gap-16 md:gap-32 h-full"><div className="w-24 h-10 bg-red-400 rounded-full rotate-12 shadow-[0_0_30px_red]" /><div className="w-24 h-10 bg-red-400 rounded-full -rotate-12 shadow-[0_0_30px_red]" /></div>;
      default: return <div className="flex gap-16 md:gap-32"><div className={`w-24 h-24 ${cyanBase}`} /><div className={`w-24 h-24 ${cyanBase}`} /></div>;
    }
  };

  const DigitalLegs = () => (
    <div className="absolute -bottom-16 left-0 w-full flex justify-around px-8">
        <div className={`w-6 h-12 bg-cyan-500/40 rounded-full border-2 border-cyan-400/20 ${isWalking ? 'animate-bounce' : ''}`} />
        <div className={`w-6 h-12 bg-cyan-500/40 rounded-full border-2 border-cyan-400/20 ${isWalking ? 'animate-bounce delay-150' : ''}`} />
    </div>
  );
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div 
        style={{ 
          ...
          transition: isChaosMode ? 'transform 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' : 'all 0.5s ease'
        }}
        className={`relative z-[100] portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black flex flex-col items-center justify-center`}
      >
        {isChaosMode && <DigitalLegs />}
        
        {isSpeaking && <div className="absolute top-12 left-1/2 -translate-x-1/2 flex items-center gap-2 text-cyan-400/60 animate-pulse"><AudioLines size={16} /><span className="text-[10px] font-mono tracking-widest uppercase">Chaos Mode Active</span></div>}
        
        <div onMouseMove={(e) => e.buttons === 1 && handlePet()} onTouchMove={handlePet} className="hidden landscape:block absolute inset-0 z-50 cursor-pointer" />
        
        <div className="w-full h-full flex items-center justify-center">{renderFace()}</div>
        
        {/* The target settings button 🎯 */}
        <button 
            onClick={() => setShowSettings(true)} 
            className={`absolute bottom-6 right-10 p-4 bg-white/5 rounded-full hover:bg-white/10 transition-all ${isChaosMode ? 'animate-pulse scale-125 border border-cyan-500/50' : 'opacity-20'}`}
        >
            <Settings size={24} className={isChaosMode ? 'text-cyan-400' : ''}/>
        </button>
      </div>

      {/* CHAT UI (BREAKS IN CHAOS) 💔 */}
      <div className={`w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden transition-all duration-700 ${isChaosMode ? 'opacity-20 blur-sm pointer-events-none skew-y-6 scale-90 translate-y-20' : ''}`}>
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          <div className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 opacity-50"><Zap size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Power</span></div>
          <div className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 opacity-50"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></div>
          <div className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 opacity-50"><Volume2 size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Audio</span></div>
        </div>
      </div>

      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[200] flex items-center justify-center p-6 text-center font-sans">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2 text-left">
                  <button onClick={() => setIsChaosMode(!isChaosMode)} className={`w-full flex items-center justify-between px-6 py-3 rounded-xl text-xs font-bold border transition-all ${isChaosMode ? 'bg-orange-600/20 border-orange-500/50 text-orange-200' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  ...
                  <button onClick={startCamera} className={`w-full flex items-center justify-between px-6 py-3 rounded-xl text-xs font-bold border transition-all ${visionEnabled ? 'bg-cyan-600/20 border-cyan-500/50 text-cyan-200' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                    <div className="flex items-center gap-2"><Eye size={14}/> Front Vision</div>
