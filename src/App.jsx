...
export default function App() {
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Analyze this image from my FRONT camera. If you see a face, eyes, hair, or skin, reply 'holding'. If you see a ceiling, fan, or roof, reply 'ceiling'. ONLY if you see a flat surface like a desk, reply 'desk'. Reply with ONLY one word." },
  ...
  useEffect(() => {
    let visionTimer;
    if (visionEnabled) visionTimer = setInterval(analyzeEnvironment, 6000);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        ...
        speak(`Hey ${name}! Eilo is back and better than ever! 🎀`);
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet robot with blue eyes. Logan Baez is your creator. Currently you see ${cameraState}. ${logan ? "Your current user is your creator Logan Baez!" : `Your current user is ${userName}.`} Be sassy and independent. NO skulls/crying.` }] } 
                })
            });
            const data = await response.json();
    ...
    return (
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="w-full max-w-sm bg-[#161622] border border-white/5 rounded-[50px] p-10 shadow-2xl animate-in fade-in zoom-in duration-700">
            ...
            <p className="mt-8 text-[10px] text-slate-500 uppercase tracking-widest font-bold">A Logan Baez Original Project</p>
        </div>
      </div>
    );
  }
