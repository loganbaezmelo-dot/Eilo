  ...
  const handleSend = async (manual) => {
    ...
    try {
      ...
      if (tempApiKey) {
        ...
        STRICT: Never sound like an AI. Never use "human", "model", "efficiency", "task". No 😭 or 💀. Use 🎀, ✨, 🎈.`;
      ...
      <div className="fixed inset-0 bg-black flex items-center justify-center overflow-hidden">
        ...
        <div className="w-full h-full flex items-center justify-center relative">
            ...
            <div className="absolute bottom-8 right-8 z-[100]">
                <button onClick={() => setShowSettings(true)} className="p-6 rounded-full border border-white/10 bg-white/5 text-slate-500 hover:bg-white/10 hover:text-white transition-all"><Settings size={32} /></button>
            </div>
            
            {aiAgentMode && (
                <div className="absolute top-8 left-1/2 -translate-x-1/2 text-[10px] font-mono text-cyan-400 uppercase tracking-widest bg-cyan-950/40 px-6 py-2 rounded-full border border-cyan-500/20 z-[100]">
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[2000] flex items-center justify-center p-6 text-center font-sans">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl flex flex-col max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-4 pb-6">
                ...
                <button onClick={startCamera} className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${visionEnabled ? 'bg-cyan-500/20 border-cyan-400/40' : 'bg-white/5 border-white/10 text-slate-400'}`}><div className="flex items-center gap-3"><Eye size={16} className={visionEnabled ? 'text-cyan-400' : ''}/> <div className="text-left"><p className="text-xs font-bold text-white">Selfie Scanner</p><p className="text-[9px] opacity-60">Visual awareness</p></div></div>{visionEnabled ? <ToggleRight size={24} className="text-cyan-400"/> : <ToggleLeft size={24}/>}</button>
