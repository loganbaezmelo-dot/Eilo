...
export default function App() {
  ...
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      // Shake logic 💀
      const threshold = 35;
      const isShaking = Math.abs(acc.x) > threshold || Math.abs(acc.y) > threshold || Math.abs(acc.z) > threshold + 10;
      setSensorHolding(Math.abs(acc.x) > 8 || Math.abs(acc.y) > 8);

      if (isShaking && mood !== 'dizzy' && mood !== 'scared') {
        setMood('dizzy');
        const name = getCurrentName();
        speak(`Whoa! ${name}, stop shaking me! I'm dizzy! 😭💀`);
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex gap-16 md:gap-32 animate-[spin_0.5s_infinite_linear]"><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /><div className="w-20 h-20 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /></div>;
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={() => {setMood('happy'); speak("Hehe! Stop tapping me!");}} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95 transition-all shadow-lg"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
