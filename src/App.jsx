...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Wheee! I'm running away! Watch out Logan, I'm gonna crush your screen! ✨🎈");
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["Eilo.legs = true", "System.overload()", "Logan.GOAT = 1", "UI_STOMPED", "PHYSICS_FAIL", "VOID_LEAK"];
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex gap-10 animate-spin"><div className="w-16 h-16 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /><div className="w-16 h-16 border-8 border-cyan-400 border-t-transparent rounded-full shadow-[0_0_30px_cyan]" /></div>;
      case 'scared': return <div className="flex gap-10 animate-bounce"><div className="w-16 h-10 bg-white rounded-full shadow-[0_0_40px_white]" /><div className="w-16 h-10 bg-white rounded-full shadow-[0_0_40px_white]" /></div>;
      case 'happy': return <div className="flex gap-10 relative"><div className="absolute -top-10 left-1/2 -translate-x-1/2"><Heart size={28} className="text-pink-400 animate-bounce fill-pink-400" /></div><div className="w-20 h-14 bg-cyan-400 rounded-full animate-bounce flex items-center justify-center shadow-lg"><div className="w-6 h-6 bg-white/30 rounded-full" /></div><div className="w-20 h-14 bg-cyan-400 rounded-full animate-bounce flex items-center justify-center shadow-lg"><div className="w-6 h-6 bg-white/30 rounded-full" /></div></div>;
      default: return <div className="flex gap-10"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div>;
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      {/* --- TOP ZONE (Fixed height Face Slot) --- */}
      <div className="w-full max-w-xl p-4 h-72 flex-shrink-0 relative">
        <div className={`w-full h-full rounded-[50px] bg-[#161622] border-2 border-white/5 flex flex-col items-center justify-center overflow-hidden transition-all duration-500 ${isChaosMode ? 'bg-black/90 border-cyan-500/20' : ''}`}>
           {isChaosMode ? (
              <div className="w-full h-full p-6 font-mono text-[10px] text-cyan-500/40 opacity-70">
                ...
                <div className="mt-2 text-cyan-400 font-bold tracking-widest animate-pulse">HARDWARE_MISMATCH: LEGS_DETECTED</div>
              </div>
           ) : (
             <div className="w-full h-full flex flex-col items-center justify-center relative">
               <div className="absolute top-4 text-[8px] font-mono tracking-widest text-white/10 uppercase">Soul Core Active</div>
               {renderFace()}
      ...
      {isChaosMode && (
        <div 
          ...
          className="bg-[#161622] border-2 border-cyan-500/30 rounded-[50px] flex flex-col items-center justify-center shadow-[0_0_60px_rgba(34,211,238,0.2)] pointer-events-auto"
        >
      ...
      {/* --- MAIN INTERFACE (Square Scrollable Chat + Bottom Buttons) --- */}
      <div 
        className={`w-full max-w-sm px-4 flex flex-col gap-4 pb-6 transition-all duration-1000 relative z-10
        ${isChaosMode ? 'skew-x-6 rotate-2 blur-[1.5px] brightness-75 scale-95 opacity-80' : ''}`}
      >
        {/* Crack Overlay */}
        {isChaosMode && (
            <div className="absolute inset-0 z-50 pointer-events-none opacity-40 mix-blend-screen overflow-hidden">
                ...
                <div className="absolute top-40 right-0 w-full h-0.5 bg-white/30 -rotate-[15deg] scale-x-150" />
                <div className="absolute bottom-20 left-10 w-full h-1 bg-white/20 rotate-[80deg] scale-x-150" />
            </div>
        )}

        {/* Square Scrollable Chat Container */}
        <div className="w-full aspect-square bg-[#161622] rounded-[40px] border border-white/5 p-5 flex flex-col overflow-hidden shadow-2xl">
          <div className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar">
            {messages.length === 0 && <div className="text-slate-600 text-[10px] text-center mt-10 uppercase tracking-widest font-mono">No data logs found</div>}
            {messages.map((m, i) => (
        ...
        {/* Action Buttons (Always Visible Below Chat) */}
        <div className="grid grid-cols-3 gap-3">
          ...
          <button onClick={() => { if(!isChaosMode) { setMood('happy'); speak("Yay! Taps!"); setTimeout(()=>setMood('neutral'), 3000); }}} className="p-3.5 rounded-[25px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95"><Hand size={16}/><span className="text-[7px] uppercase font-bold tracking-widest">Pet</span></button>
