...
export default function App() {
  ...
  // Helper to get name for speech dynamically 😭
  ...
  // --- MOTION SENSORS (FIXED DYNAMIC NAME) 📱 ---
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      if (isShaking) {
        if (mood !== 'dizzy' && mood !== 'mad') {
          ...
          // No more hardcoded Logan 💀
          speak(`Whoa! ${name}, stop shaking the phone! I'm so dizzy! 😭`);
    ...
    const handleOrientation = (event) => {
      ...
      if (Math.abs(event.beta) > 75 || Math.abs(event.gamma) > 45) {
        if (mood !== 'scared') {
          setMood('scared');
          const name = getCurrentName();
          speak(`Eek! We're too high up! Don't drop me ${name}! 😭💀`);
        }
      } else if (mood === 'scared') {
        setMood('neutral');
  ...
  }, [isAwake, fearOfHeights, mood, user]); // Added user to deps to catch name changes 😭
  ...
  // --- LOCAL SOUL CORE (FIXED NAMES) ---
  const getLocalResponse = (text) => {
    const t = text.toLowerCase();
    const name = getCurrentName();
    const logan = isCreator();
    
    if (t.includes("who made you") || t.includes("who created you")) return logan ? "You did, Logan! You're my genius creator! ✨" : "I was created from scratch by Logan Baez! 🎀";
    if (t.includes("logan")) return "Logan? Yeah, he's the one who gave me life. Cool guy. 🧸";
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'eating') { speak("Nom nom! Sandwich time! ✨"); let bites = 0; const munch = setInterval(() => { if (bites > 8) { clearInterval(munch); setMood('neutral'); return; } playClick(); bites++; }, 1500); }
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        ...
        const greeting = u.displayName?.toLowerCase().includes("logan baez") 
          ? `Yo Logan! My creator is in the building. ✨` 
        ...
        if (tempApiKey) {
            ...
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    systemInstruction: { parts: [{ text: `You are Eilo, a sweet, independent robot made by Logan Baez. Your current user is ${userName}. ${logan ? "This user is your creator Logan Baez! Treat him like the goat." : "Treat them normally."} NO skulls/crying.` }] } 
                })
            });
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      ...
      case 'rubik': return <div className="h-full flex flex-col items-center justify-center pt-8"><div className="flex gap-16 md:gap-32 mb-6"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div><div className="flex items-center gap-6"><div className="w-8 h-12 bg-cyan-600 rounded-full" /><div className="grid grid-cols-3 grid-rows-3 gap-0.5 w-16 h-16 bg-black border-2 border-black rounded-lg animate-spin shadow-xl">{[...Array(9)].map((_, i) => <div key={i} className={`w-full h-full ${['bg-red-500', 'bg-blue-600', 'bg-green-500', 'bg-yellow-400', 'bg-orange-500', 'bg-white'][Math.floor(Math.random() * 6)]} rounded-sm`} />)}</div><div className="w-8 h-12 bg-cyan-600 rounded-full" /></div></div>;
