...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, MicOff, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, ShieldCheck, LogOut, Radio, Network, Cpu
} from 'lucide-react';
...
export default function App() {
  ...
  const [isHandBlocking, setIsHandBlocking] = useState(false); // THE EVIL BLOCKER ✋💀
  ...
  useEffect(() => {
    ...
    speak("Wheee! I'm running away! Try to catch me! 🎈✨");
    ...
    // THE EVIL BLOCKING CYCLE 😈
    let blockTimeout;
    const startBlockingCycle = () => {
        setIsHandBlocking(true); // Block immediately
        // Wait 45 seconds 💀
        blockTimeout = setTimeout(() => {
            setIsHandBlocking(false); // Unblock
            speak("Ugh... my arm is tired... quick! 🧸");
            // Wait 3.5 seconds then block again 😭
            blockTimeout = setTimeout(() => {
                speak("Haha! Too slow! Blocked again! 🎀");
                startBlockingCycle();
            }, 3500);
        }, 45000);
    };
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' 
      ? [`Logan!! Put me down! 🎈`, `Don't drop me! ✨`]
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Respond 'holding' if face/hands. Respond 'ceiling' if roof. Respond 'desk' if flat surface. ONE WORD." },
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      {isChaosMode && (
        <div style={{ transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`, transition: 'transform 1.4s cubic-bezier(0.34, 1.56, 0.64, 1)', position: 'fixed', top: '50%', left: '50%', marginTop: '-120px', marginLeft: '-40%', width: '80%', height: '14rem', zIndex: 1000 }} className="bg-[#161622] border-2 border-cyan-500/30 rounded-[50px] flex flex-col items-center justify-center shadow-2xl pointer-events-auto">
          ...
          {/* THE GIANT EVIL HAND BLOCKER ✋💀 */}
          {isHandBlocking && (
             <div 
                className="absolute -bottom-8 -right-8 z-[200] animate-bounce cursor-not-allowed pointer-events-auto" 
                onClick={handleBlockedClick}
             >
                <div className="text-[10rem] drop-shadow-2xl hover:scale-105 transition-transform rotate-12 filter grayscale-[0.2]">✋</div>
             </div>
          )}
