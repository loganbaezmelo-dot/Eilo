...
export default function App() {
  ...
  const speak = async (text) => {
    ...
    try {
      ...
      // Manual WAV Header Construction for 24000Hz PCM
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') speak("Zzz... Honk... Mimimi... I'm a sleepy robot...");
    if (choice === 'eating') speak("Nom nom! This sandwich is literally so good! ✨");
    if (choice === 'rubik') speak("Click clack! I'm almost finished with this cube! 🎈");
  };
  ...
  const handlePet = () => {
    ...
    if (['sleeping', 'eating', 'rubik'].includes(mood)) {
        setMood('mad');
        speak("HEY! Stop it! I was busy doing something important! ✨");
        setTimeout(() => setMood('neutral'), 4000);
        resetIdleTimer();
        return;
    }
    setMood('happy');
    const phrases = ["That feels so nice! ✨", "Hehe, stop it, that tickles! 🎀", "You're the best owner ever! 🧸"];
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({
                    ...
                    systemInstruction: { parts: [{ text: "You are Eilo, a sweet, energetic robot with blue eyes. Use cute emojis like ✨, 🎀, 🧸. NO skulls or crying." }] }
                })
            });
            const data = await response.json();
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm mb-2 outline-none text-center" />
                
                <button onClick={() => speak("Voice test! Can you hear me owner? ✨")} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all">
