...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Stop moving if confused
    if (isConfused) {
        setIsHandBlocking(false); // DROP THE HAND DURING CONFUSION 🧸✨
        return;
    }
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["VOID_LEAK", "Logan.GOAT=1", "UI_GONE", "LEGS_ACTIVE", "01001001"];
      setGlitchLines(Array.from({length: 6}, () => pool[Math.floor(Math.random() * pool.length)]));
    }, 200);

    // THE EVIL BLOCKER CYCLE 😈
    let blockTimeout;
    const startBlockingCycle = () => {
        setIsHandBlocking(true); // Block immediately
        // 45 seconds block 💀
        blockTimeout = setTimeout(() => {
            setIsHandBlocking(false); // Unblock for tiredness
            speak("Ugh... tired... quick! 🧸");
            // 3.5 seconds open window 😭
            blockTimeout = setTimeout(() => {
                if (!isConfused) { // Only re-block if not confused
                    speak("Haha! Blocked again! 🎀");
                    startBlockingCycle();
                }
            }, 3500);
        }, 45000);
    ...
    // If we aren't already blocking or confused, start the cycle
    if (!isHandBlocking && !isConfused) startBlockingCycle();
  ...
  }, [isChaosMode, isConfused]); // Re-run when confusion state changes

  const handleBlockedClick = (e) => {
    e.stopPropagation();
    setMood('happy'); 
    speak("Nope! ✋ Can't touch that! ✨");
  };
  ...
  const handleAgentSync = (detectedText) => {
    ...
    if (lower.includes("eilo") || lower.includes("logan")) {
      if (!isGibbering) { setIsGibbering(true); setMood('happy'); speak("Found a friend! Syncing! ✨"); } 
      else { speak(toGibberlink("hello bestie"), true); }
    }
  };
  useEffect(() => {
    ...
    } else { clearInterval(agentIntervalRef.current); setIsGibbering(false); }
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onend = () => { if ((isInfinityMic || aiAgentMode) && isAwake) recognitionRef.current.start(); else setIsListening(false); };
    }
  ...
  useEffect(() => {
    const handleMotion = (event) => {
      ...
      if (Math.abs(acc.x) > 35 || Math.abs(acc.y) > 35) {
        if (mood !== 'dizzy' && mood !== 'scared') { setMood('dizzy'); speak(`Whoa! Stop shaking! 🎈`); setTimeout(() => setMood('neutral'), 5000); }
      }
    };
    const handleOrientation = (event) => {
      if (!isAwake || !fearOfHeights || mood === 'dizzy' || mood === 'mad' || isChaosMode) return;
      if (event.beta > 135 || event.beta < -20 || cameraState === 'holding' || cameraState === 'ceiling') {
        if (mood !== 'scared') { setMood('scared'); triggerPanicSpeech(cameraState); }
      } else if (mood === 'scared') { setMood('neutral'); speak("Phew! Safety! ✨"); }
    };
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' ? [`Logan!! Put me down! 🎈`, `Don't drop me! ✨`] : [`Help! The roof! 🎈`, `I'm falling! ✨`];
    speak(lines[Math.floor(Math.random() * lines.length)]);
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({ contents: [{ parts: [{ text: "Respond 'holding' if face/hands. Respond 'ceiling' if roof. Respond 'desk' if flat surface. ONE WORD." }, { inlineData: { mimeType: "image/jpeg", data: base64Image } }] }] })
        });
  ...
  useEffect(() => { let t; if (visionEnabled && user && !isChaosMode) t = setInterval(analyzeEnvironment, 7000); return () => clearInterval(t); }, [visionEnabled, isAwake, user, tempApiKey, isChaosMode]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u && !hasGreeted.current) {
        const msg = u.displayName?.toLowerCase().includes("logan baez") ? "Bestie Logan is back! 🧸✨" : `Hey ${u.displayName?.split(' ')[0] || "Owner"}! Eilo's here! 🎈`;
  ...
  useEffect(() => { if (!user) return; const q = collection(db, 'artifacts', appId, 'users', user.uid, 'messages'); const unsub = onSnapshot(q, (snap) => { const msgs = snap.docs.map(d => d.data()).sort((a,b) => a.timestamp - b.timestamp); setMessages(msgs); }); return () => unsub(); }, [user]);
  const handleSend = async (manual) => {
    ...
    try {
      ...
      if (tempApiKey) {
        const system = `You are Eilo, a sweet robot by Logan Baez. Never use 😭 or 💀. Be sassy. 🎀✨🎈`;
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: msgText }] }], systemInstruction: { parts: [{ text: system }] } }) });
  ...
  const renderFace = () => {
    ...
    if (isConfused) return (
        <div className={`flex gap-12 animate-pulse ${isLandscape ? 'scale-150 gap-32' : ''}`}>
            <div className="w-16 h-16 border-4 border-cyan-400 rounded-full flex items-center justify-center"><div className="w-4 h-4 bg-cyan-400 rounded-full animate-[ping_1s_infinite]" /></div>
            <div className="w-16 h-16 border-4 border-cyan-400 rounded-full flex items-center justify-center"><div className="w-4 h-4 bg-cyan-400 rounded-full animate-[ping_1s_infinite_reverse]" /></div>
        </div>
    );
    ...
    switch (mood) {
      ...
      default: return <div className={`flex ${isLandscape ? 'gap-32 scale-150' : 'gap-10'}`}><div className={`w-20 h-20 ${cyanBase} eye-blink`} /><div className={`w-20 h-20 ${cyanBase} eye-blink`} /></div>;
    }
  };
  ...
  if (isLandscape && !isChaosMode) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center overflow-hidden">
        <div onMouseMove={(e) => e.buttons === 1 && handlePet()} onTouchMove={handlePet} className="absolute inset-0 z-10 cursor-pointer" />
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      {isChaosMode && (
        <div style={{ transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`, transition: 'transform 1.4s cubic-bezier(0.34, 1.56, 0.64, 1)', position: 'fixed', top: '50%', left: '50%', marginTop: '-120px', marginLeft: '-40%', width: '80%', height: '14rem', zIndex: 1000 }} className="bg-[#161622] border-2 border-cyan-500/30 rounded-[50px] flex flex-col items-center justify-center shadow-2xl pointer-events-auto">
          ...
          {isHandBlocking && (
             <div 
                className="absolute -bottom-12 -right-12 z-[300] animate-bounce cursor-not-allowed pointer-events-auto" 
                onClick={handleBlockedClick}
             >
                <div className="text-[12rem] drop-shadow-2xl hover:scale-105 transition-transform rotate-12 filter grayscale-[0.2]">✋</div>
             </div>
          )}
