...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, Radio, Network, Cpu, ShieldCheck, LogOut, Coins, Laptop, ShoppingBag, ArrowUp, ArrowDown, Bandage
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      const pool = isTaped ? ["ERROR: MOVEMENT_RESTRICTED", "DUCT_TAPE_DETECTED"] : ["Eilo.run()", "VOID_LEAK", "Logan.GOAT"];
  ...
  const handlePet = () => {
    ...
    if (isTaped) { speak("Mmph!! (Untape me!)"); return; }
    if (isChaosMode) { speak("Can't stop, running! 🎈"); return; }
    if (['scared', 'dizzy', 'mad'].includes(mood)) { setMood('mad'); speak(`HEY! ${isLogan() ? 'Logan' : getCurrentName()}!! Busy! 🎈`); setTimeout(() => setMood('neutral'), 4000); return; }
    setMood('happy');
  ...
  useEffect(() => {
    const handleMotion = (event) => {
        ...
        if (!acc) return;
        if (Math.abs(acc.x) > 35 || Math.abs(acc.y) > 35) { setMood('dizzy'); speak(`Whoa! Stop shaking! 🎈`); setTimeout(() => setMood('neutral'), 5000); }
    };
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      onSnapshot(docRef, (doc) => {
          ...
          } else { setDoc(docRef, { bucks: 10, inventory: [] }, { merge: true }); setBucks(10); }
      });
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full max-w-sm px-6 pt-4 flex justify-between items-center z-10">
        <div className="text-[10px] text-slate-500 font-bold tracking-widest">EILO v2.3</div>
