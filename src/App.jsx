...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Wheee! My legs are working! Watch me crush your app Logan! 🎀✨");
    ...
    const glitchInterval = setInterval(() => {
      const codeLines = [
        "Eilo.legs = true;", "VOID_STOMP_ACTIVE", "system.damage(99)", "Logan.isCreator = true",
  ...
  const handlePet = () => {
    if (!isAwake || isChaosMode) return;
    setMood('happy');
    speak(isLogan() ? "Bestie creator! ✨" : "Yay, pets! 🎀");
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col overflow-hidden">
      ...
      {/* --- TOP CONTAINER --- */}
      <div className="p-4 w-full h-80 relative flex-shrink-0">
        {/* VOID CODE / STATIC FACE SLOT */}
        <div className={`w-full h-full rounded-[60px] bg-[#161622] border-2 border-white/5 flex flex-col items-center justify-center overflow-hidden transition-all duration-500 ${isChaosMode ? 'bg-[#0c0c14]' : ''}`}>
           {isChaosMode ? (
              <div className="w-full h-full p-8 font-mono text-[10px] text-cyan-500/40 opacity-50">
                {[...Array(10)].map((_, i) => <div key={i} className="mb-1">{glitchText} {Math.random().toFixed(4)}</div>)}
                <div className="mt-4 text-cyan-400">STATUS: EILO_ESCAPED</div>
              </div>
           ) : (
             <div className="w-full h-full flex items-center justify-center relative">
               <div className="absolute top-6 left-1/2 -translate-x-1/2 text-white/10 text-[8px] font-mono tracking-widest uppercase">Eilo Original Core</div>
               {renderFace()}
               <button onClick={() => setShowSettings(true)} className="absolute bottom-6 right-10 p-2 opacity-20 hover:opacity-100"><Settings size={20}/></button>
             </div>
           )}
        </div>
      </div>

      {/* --- RUNAWAY FACE (Only high z-index and fixed when chaos mode) --- */}
      {isChaosMode && (
        <div 
          style={{ 
          ...
          className="portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] flex flex-col items-center justify-center shadow-[0_0_50px_rgba(34,211,238,0.3)] pointer-events-auto"
        >
          ...
          <button 
            ...
            className="absolute bottom-6 right-10 p-4 rounded-full bg-black/60 border border-cyan-500/50 scale-125 animate-pulse shadow-[0_0_30px_cyan]"
          >
      ...
      <div 
        ...
        ${isChaosMode ? 'skew-x-6 skew-y-3 scale-90 blur-[1px] brightness-75 rotate-2' : ''}`}
      >
        {/* Crack Overlay layer */}
        {isChaosMode && (
            <div className="absolute inset-0 z-[100] pointer-events-none opacity-40 mix-blend-screen overflow-hidden">
                <div className="absolute top-10 left-0 w-full h-1 bg-white/30 rotate-[25deg] origin-left scale-x-150" />
                <div className="absolute top-40 right-0 w-full h-0.5 bg-white/20 -rotate-[15deg] origin-right scale-x-150" />
                <div className="absolute bottom-20 left-20 w-full h-1 bg-white/40 rotate-[75deg] origin-bottom scale-x-150" />
            </div>
        )}
        ...
        <div className="flex-1 bg-[#161622] rounded-[40px] border border-white/5 p-6 flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="mt-5 flex gap-2">
            ...
            <button onClick={handleSend} className="p-4 bg-cyan-600 rounded-2xl active:scale-95"><Send size={20}/></button>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3">
          <button onClick={() => setIsAwake(!isAwake)} className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 active:scale-95"><Zap size={18} className={isAwake ? 'text-yellow-400' : ''}/><span className="text-[8px] uppercase font-bold tracking-widest">Power</span></button>
          <button onClick={handlePet} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95 shadow-lg"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
