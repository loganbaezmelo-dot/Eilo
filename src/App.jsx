...
export default function App() {
  ...
  // --- VISION CORE (Aggressive Detection) 👁️ ---
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [
                { text: "You are a robot's eye. This is a FRONT camera view. If you see a human face, a person, eyes, or skin, reply 'holding'. If you see a ceiling, ceiling fan, lights, or the sky, reply 'ceiling'. ONLY if you see a flat, inanimate, stationary surface like a desk, table, or empty floor with NO human parts, reply 'desk'. Be extremely strict. Reply with ONLY one word." },
        ...
        if (result === 'holding' || result === 'ceiling') {
          setMood('scared');
          if (result === 'holding') {
            speak(`I see you ${name}! You're holding me! Don't let me fall! 😭💀`);
          } else {
            speak(`Eek! I'm looking at the ceiling! Put me back down! 😭`);
          }
        }
  ...
  useEffect(() => {
    ...
    const handleOrientation = (event) => {
      ...
      // Strict panic trigger
      if (lookingDown || cameraState !== 'desk') {
        if (mood !== 'scared') {
          setMood('scared');
        }
      } else if (mood === 'scared' && !lookingDown && cameraState === 'desk') {
        // Only relax if BOTH sensors and camera agree it's a desk 🧸✨
        setMood('neutral');
        speak("Phew! Safety first! Back on my favorite surface. ✨");
      }
    };
    ...
    try {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ contents: [{ parts: [{ text: userMsg }] }], systemInstruction: { parts: [{ text: `You are Eilo, an independent blue-eyed robot created by Logan Baez. Your creator is Logan Baez. You are sassier than a regular AI. NO skulls/crying.` }] } })
            });
            const data = await response.json();
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      if (u && !hasGreeted.current) {
        speak(`Hey ${u.displayName?.split(' ')[0]}! Eilo's online. 🎀`);
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      ...
      case 'scared': return <div className="flex gap-16 md:gap-32 animate-bounce"><div className="w-20 h-12 bg-white rounded-full shadow-[0_0_30px_white]" /><div className="w-20 h-12 bg-white rounded-full shadow-[0_0_30px_white]" /></div>;
    ...
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden">
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          <button onClick={() => setIsAwake(!isAwake)} className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 active:scale-95 transition-all font-sans"><Zap size={18} className={isAwake ? 'text-yellow-400' : ''}/><span className="text-[8px] uppercase font-bold tracking-widest">Power</span></button>
