...
export default function App() {
  ...
  const triggerIdleAction = () => {
    ...
    if (choice === 'sleeping') {
      speak("Zzz. I am a sleepy robot. Zzz.");
      setTimeout(() => setMood('neutral'), 10000); // Longer nap
    }
    if (choice === 'eating') {
      speak("Nom nom nom! This sandwich is so yummy! ✨");
      // Simulate multiple bites
      let bites = 0;
      const munch = setInterval(() => {
        if (bites > 5) { clearInterval(munch); setMood('neutral'); return; }
        playClick(); // Sound of chewing
        bites++;
      }, 1500);
    }
    if (choice === 'rubik') {
      speak("Click clack! This puzzle is so hard! 🧩");
      let moves = 0;
      const solve = setInterval(() => {
        if (moves > 8) { clearInterval(solve); setMood('neutral'); return; }
  ...
  const handlePet = () => {
    ...
    if (['sleeping', 'eating', 'rubik'].includes(mood)) {
        setMood('mad');
        speak("HEY! Stop it! I was busy! 😭");
        setTimeout(() => { setMood('neutral'); resetIdleTimer(); }, 4000);
        return;
    }

    setMood('happy');
    speak(["That feels so nice! ✨", "Hehe, tickles! 🎀", "I love you owner! 🧸"][Math.floor(Math.random() * 3)]);
    ...
    try {
        ...
        if (tempApiKey) {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
                ...
                body: JSON.stringify({ contents: [{ parts: [{ text: userMsg }] }], systemInstruction: { parts: [{ text: "You are Eilo, a sweet robot with blue eyes. Use cute emojis like ✨, 🎀. NO skulls/crying." }] } })
            });
  ...
  // --- RUBIKS CUBE COMPONENT ---
  const RubiksCube = () => (
    <div className="grid grid-cols-3 grid-rows-3 gap-0.5 w-16 h-16 bg-black border-2 border-black rounded-lg animate-spin transition-all duration-700">
      {[...Array(9)].map((_, i) => {
        const colors = ['bg-red-500', 'bg-blue-500', 'bg-green-500', 'bg-yellow-400', 'bg-orange-500', 'bg-white'];
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[100] flex items-center justify-center p-6 text-center">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                ...
                <button onClick={() => speak("Voice test! I sound super high pitched and cute! ✨")} className="w-full flex items-center justify-center gap-2 bg-white/5 border border-white/10 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all"><Mic size={14}/> Test My Voice</button>
