...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Whoops! Logan, I'm escaping! Check out these legs! 🏃‍♂️✨");
    ...
    const glitchInterval = setInterval(() => {
      const codeLines = [
        "Eilo.run();", "UI.collapse()", "Logan.GOAT = true;", "System.delete('Buttons')",
        "01100110 01110101 01101110", "AI_CORE: LEGS_ATTACHED", "ERROR: VOID_DETECTED"
      ];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Respond 'holding' if face/hands. Respond 'ceiling' if roof. Respond 'desk' if empty flat surface. ONE WORD." },
  ...
  const handlePet = () => {
    if (!isAwake || isChaosMode) return;
    setMood('happy');
    speak(isLogan() ? "Hehe, creator! ✨" : "Yay, petting! 🎀");
    ...
    return (
      <div className="min-h-screen bg-[#0c0c14] text-white flex flex-col items-center justify-center p-6 text-center font-sans">
        <div className="w-full max-w-sm bg-[#161622] border border-white/5 rounded-[50px] p-10 shadow-2xl animate-in fade-in zoom-in duration-700">
            ...
            <button onClick={() => signInWithPopup(auth, googleProvider)} className="w-full bg-white text-black py-5 rounded-2xl font-bold active:scale-95 transition-all shadow-xl text-lg">Google Sync ✨</button>
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-4 left-4 w-[92%] h-72 rounded-[60px] bg-[#0c0c14] border-2 border-white/5 flex flex-col items-center justify-center overflow-hidden z-0">
        {isChaosMode && (
          <div className="w-full h-full p-10 font-mono text-[10px] text-cyan-500/30">
      ...
      <div 
        style={{ 
          ...
          zIndex: isChaosMode ? 200 : 50 // High when chaos, but lowered when idle
        }}
      ...
      {/* --- CHAT UI --- */}
      <div className={`w-full max-w-xl px-4 mt-80 flex flex-col gap-4 h-[440px] transition-all duration-1000 ${isChaosMode ? 'opacity-0 pointer-events-none scale-0 blur-3xl' : 'z-10'}`}>
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          ...
          <button onClick={handlePet} className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 active:scale-95"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></button>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-6 text-center font-sans">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl">
            ...
            <div className="space-y-4">
                <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Paste Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm outline-none text-center focus:border-cyan-500/50 text-white" />
                ...
                <button onClick={() => { localStorage.setItem('eilo_key', tempApiKey); setShowSettings(false); }} className="w-full bg-cyan-600 py-4 rounded-2xl font-bold uppercase active:scale-95 text-white">Update AI</button>
