...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand 
} from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[9999] flex items-center justify-center p-6 text-center font-sans">
      <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <h2 className="text-xl font-bold mb-6 flex items-center justify-center gap-2 text-white">Settings ✨</h2>
            ...
            <div>
               ...
               <div className="space-y-2">
                  ...
                  <button onClick={() => buyItem(150, 'rogue_walk')} disabled={safeInv.includes('rogue_walk')} className={`w-full flex items-center justify-between p-3 rounded-2xl border transition-all ${safeInv.includes('rogue_walk') ? 'bg-green-500/20 border-green-500/40 opacity-50' : 'bg-white/5 border-white/10 hover:border-yellow-500/50'}`}>
                     <div className="flex items-center gap-3"><span className="text-lg">👻</span> <div><p className="text-xs font-bold text-white">Rogue Legs</p><p className="text-[9px] text-slate-500">Walks without Chaos Mode</p></div></div>
        ...
        <div className="pt-4 border-t border-white/5 space-y-3">
            ...
            <button onClick={handleSignOut} className="w-full flex items-center justify-center gap-2 text-[10px] text-red-500 font-bold uppercase opacity-60 hover:opacity-100 py-2 transition-opacity">Disconnect Core</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const speak = (text, isRobotLang = false) => {
    ...
    // Muffled tape sounds with periods to prevent spelling out letters
    let finalText = text;
    if (isTaped) {
  ...
  const getLocalResponse = (text) => {
    ...
    const randoms = [
      ...
      "I wonder if other robots have legs like mine?",
      `You're the best! 🧸`,
  ...
  useEffect(() => {
      ...
      if (mood === 'sleeping' && isAwake) {
          napTimer = setTimeout(() => {
              setMood('happy');
              const msg = isLogan() ? `Yawn! Thanks for letting me sleep, Logan!` : `Yawn! That was a good nap!`;
              speak(msg);
  ...
  const toggleMic = () => { speak("Mic toggle pressed"); }; // Simplified for stability
