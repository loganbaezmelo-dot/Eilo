...
const SettingsOverlay = ({ 
  ...
  // CRITICAL FIX: Ensure inventory is never null/undefined
...
export default function App() {
  ...
  // Economy & Inventory (Defaults to empty array)
  ...
  // --- SPEAK FUNCTION (FIXED FOR TAPE) ---
  const speak = (text, isRobotLang = false) => {
    if (isMuted || !isAwake) return;
    setIsSpeaking(true);
    window.speechSynthesis.cancel();
    
    // Check tape state HERE.
    const speechText = isTaped ? "Hmph! Mmm! Mmph!" : text;
  ...
  const applyDuctTape = () => {
      ...
      // Manually trigger specific voice lines based on the NEW state
      window.speechSynthesis.cancel();
      if (newState) {
          // Applying tape
          const utterance = new SpeechSynthesisUtterance("Hmph! Mmm! Mmph!");
          utterance.pitch = 0.5; utterance.rate = 0.8;
          window.speechSynthesis.speak(utterance);
      } else {
          // Removing tape
          const utterance = new SpeechSynthesisUtterance("I'm free! Never do that again! 🎀");
  ...
  const handleSend = async (manual) => {
    ...
    if(isTaped) { speak("Mmmph!"); return; } // Cannot chat if taped
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full max-w-sm px-6 pt-4 flex justify-between items-center z-10">
        <div className="text-[10px] text-slate-500 font-bold tracking-widest">EILO v2.7</div>
