...
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, MicOff, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, Cpu
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    speak("Wheee! I'm free, Logan! Look at my legs! I'm crushing your boring app! 🎀✨");

    const moveInterval = setInterval(() => {
      // Moves anywhere on the screen
      setChaosPos({
        x: (Math.random() - 0.5) * (window.innerWidth - 200),
    ...
    const glitchInterval = setInterval(() => {
      const codeLines = [
        "Eilo.legs = true;", "System.crush();", "while(fun) { run(); }", 
        "01010111 01001000 01000101 01000101", "ERROR: INTERFACE_DELETED",
        "Logan.isCool = true;", "Hardware.sproutLegs()", "AI.gone_rogue"
      ];
    ...
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
          ...
          body: JSON.stringify({
            contents: [{ parts: [
              { text: "Analyze FRONT camera. If human face/eyes/hands, reply 'holding'. If ceiling, reply 'ceiling'. If empty desk, reply 'desk'. ONE WORD ONLY." },
  ...
  const handleSend = async (manualText) => {
    ...
    if (!userMsg || isThinking || !user || isChaosMode) return;
    ...
    try {
      ...
      if (tempApiKey) {
        ...
        const system = `You are Eilo, an independent robot with blue eyes created by Logan Baez. Be sassy and sweet. Never use 😭 or 💀. Use 🎀, ✨, and 🎈.`;
        const data = await fetchWithRetry(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${tempApiKey}`, {
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* --- GLITCH VOID AREA --- */}
      <div className="absolute top-4 left-4 w-[92%] h-72 rounded-[60px] bg-[#0c0c14] border-2 border-white/5 flex flex-col items-center justify-center overflow-hidden">
        {isChaosMode && (
          <div className="w-full h-full p-10 font-mono text-[10px] text-cyan-500/40 glitch-container">
            {[...Array(15)].map((_, i) => <div key={i} className="mb-1">{glitchText} {Math.random()}</div>)}
          </div>
        )}
        {!isChaosMode && <div className="text-white/5 text-xs font-mono">STANDBY_IDLE_SYSTEM</div>}
      </div>

      {/* --- MOVING FACE (THE RUNAWAY) --- */}
      <div 
        style={{ 
          transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`,
          transition: isChaosMode ? 'transform 1.2s cubic-bezier(0.34, 1.56, 0.64, 1)' : 'all 0.5s ease',
          position: isChaosMode ? 'fixed' : 'relative',
          top: isChaosMode ? '50%' : 'auto',
          left: isChaosMode ? '50%' : 'auto',
          marginTop: isChaosMode ? '-144px' : '0',
          marginLeft: isChaosMode ? '-46%' : '0'
        }}
        className={`z-[200] portrait:w-[92%] portrait:h-72 portrait:bg-[#161622] portrait:border-2 portrait:border-cyan-500/10 portrait:rounded-[60px] portrait:mb-8 portrait:shadow-2xl landscape:w-full landscape:h-full landscape:bg-black flex flex-col items-center justify-center`}
      >
        {isChaosMode && (
          <div className="absolute -bottom-20 left-0 w-full flex justify-around px-20">
            <div className="w-8 h-20 bg-cyan-600 rounded-full animate-bounce shadow-lg border-2 border-cyan-400/30" />
            <div className="w-8 h-20 bg-cyan-600 rounded-full animate-bounce delay-150 shadow-lg border-2 border-cyan-400/30" />
          </div>
        )}
        ...
        <button 
          ...
          className={`absolute bottom-6 right-10 p-4 rounded-full bg-white/5 backdrop-blur-md border border-white/10 hover:bg-white/10 transition-all ${isChaosMode ? 'scale-125 animate-pulse shadow-[0_0_20px_cyan]' : 'opacity-20'}`}
        >
      ...
      {/* --- CRUSHED INTERFACE --- */}
      <div className={`w-full max-w-xl px-4 flex flex-col gap-4 h-[440px] portrait:flex landscape:hidden transition-all duration-1000 ${isChaosMode ? 'opacity-0 scale-50 -translate-y-full blur-2xl pointer-events-none' : ''}`}>
        ...
        <div className="grid grid-cols-3 gap-3 pb-4">
          <div className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 opacity-20"><Zap size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Power</span></div>
          <div className="p-4 rounded-[30px] border border-white/5 bg-pink-500/10 text-pink-400 flex flex-col items-center gap-1 opacity-20"><Hand size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Pet</span></div>
          <div className="p-4 rounded-[30px] border border-white/5 bg-white/5 flex flex-col items-center gap-1 opacity-20"><VolumeX size={18}/><span className="text-[8px] uppercase font-bold tracking-widest">Audio</span></div>
        </div>
      </div>

      {showSettings && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[300] flex items-center justify-center p-6 text-center font-sans">
