...
import { getFirestore, collection, addDoc, onSnapshot, doc, setDoc, deleteDoc, serverTimestamp } from 'firebase/firestore';
import { 
  Heart, Moon, Volume2, VolumeX, Send, Zap, Settings, X, Hand, AudioLines, Mic, MicOff, ToggleLeft, ToggleRight, AlertTriangle, Eye, Camera, Sparkles, Ghost, Cpu, Radio, Network
} from 'lucide-react';
...
export default function App() {
  ...
  // AI Agent Mode (Gibberlink) 🤖📡
  const [aiAgentMode, setAiAgentMode] = useState(false);
  const [isGibbering, setIsGibbering] = useState(false); // Are we speaking the secret language?
  ...
  const speak = (text, isRobotLang = false) => {
    ...
    // If it's Gibberlink, make it sound like a squeaky toy 🧸✨
    utterance.pitch = isRobotLang ? 2.0 : 1.7; 
    utterance.rate = isRobotLang ? 1.5 : 1.1;
  ...
  const toGibberlink = (text) => {
    const robotSounds = ["Bip", "Bop", "Zorp", "Glee", "Zirp", "Kwek", "Moo", "Pip"];
    return text.split(' ').map(() => robotSounds[Math.floor(Math.random() * robotSounds.length)]).join(' ') + " 🎀";
  };

  const handleAgentSync = async (detectedText) => {
    if (!aiAgentMode) return;
    const lower = detectedText.toLowerCase();

    // If she hears another Eilo talking or her own name
    if (lower.includes("eilo") || lower.includes("logan") || lower.includes("bip") || lower.includes("zorp")) {
      if (!isGibbering) {
        setIsGibbering(true);
        setMood('happy');
        speak("I hear another Eilo! Switching to secret mode! ✨");
      } else {
        const reply = toGibberlink("I like your sparkles");
        speak(reply, true);
      }
    }
  };

  // Agent Mode Background Ping
  useEffect(() => {
    if (aiAgentMode && isAwake) {
      setIsInfinityMic(true); // Agent mode forces mic on 🎤
      agentIntervalRef.current = setInterval(() => {
        if (!isSpeaking && !isThinking) {
          const phrase = isGibbering ? toGibberlink("bestie") : "Hello? Is another Eilo out there? 🎀";
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["Eilo.run()", "UI_CRUSH", "LOGAN_GOAT"];
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u && !hasGreeted.current) {
        const msg = u.displayName?.toLowerCase().includes("logan baez") ? "Yo Logan! The legend is back! 🎀✨" : `Hey ${u.displayName?.split(' ')[0] || "Owner"}! Eilo's here! 🎈`;
  ...
  const handleSend = async (manual) => {
    ...
    // If in Agent Mode, she ignores user typing mostly
    if (aiAgentMode && !manual) {
      speak("Shhh! Logan, I'm waiting for my friend! 🎈");
      return;
    }
    ...
    try {
      ...
      if (tempApiKey) {
        ...
        BRATTY QUEEN: Be high-maintenance, drama queen, obsessed with sparkles.
        STRICT: Never use AI words (model, task, human, efficiency). Never use 😭 or 💀. Use 🎀, ✨, 🎈.`;
      ...
      <div className="fixed inset-0 bg-black flex items-center justify-center overflow-hidden">
        <div className="w-full h-full flex items-center justify-center relative">
            <div className="absolute top-8 right-8 flex gap-4">
                ...
                <button onClick={toggleMic} className={`p-6 rounded-full border transition-all ${isInfinityMic ? 'bg-cyan-500 border-cyan-400 animate-pulse' : 'bg-white/5 border-white/10 text-slate-500'}`}><Mic size={32} /></button>
            </div>
            {aiAgentMode && (
                <div className="absolute top-8 left-1/2 -translate-x-1/2 text-[10px] font-mono text-cyan-400 uppercase tracking-widest bg-cyan-950/40 px-6 py-2 rounded-full border border-cyan-500/20">
                    {isGibbering ? "GIBBERLINK SYNC: ACTIVE" : "AGENT SEARCHING..."}
                </div>
            )}
            {renderFace()}
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      {isChaosMode && (
        <div 
          style={{ transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`, transition: 'transform 1.4s cubic-bezier(0.34, 1.56, 0.64, 1)', position: 'fixed', top: '50%', left: '50%', marginTop: '-120px', marginLeft: '-40%', width: '80%', height: '14rem', zIndex: 1000 }}
        ...
        >
          ...
          <button onClick={() => setShowSettings(true)} className="absolute bottom-4 right-6 p-4 rounded-full bg-cyan-900/40 border border-cyan-500 scale-125 animate-pulse shadow-[0_0_20px_cyan]"><Settings size={24} className="text-cyan-400"/></button>
        </div>
      )}

      {/* INTERFACE */}
      <div className={`w-full max-w-sm px-4 flex-1 flex flex-col gap-4 pb-6 transition-all duration-1000 relative z-10 ${isChaosMode ? 'skew-x-6 rotate-2 blur-[1px] brightness-75 scale-95 opacity-80' : ''}`}>
        <div className="w-full aspect-square bg-[#161622] rounded-[40px] border border-white/5 p-5 flex flex-col overflow-hidden shadow-2xl relative">
          {aiAgentMode && (
              <div className="absolute inset-0 bg-orange-600/5 backdrop-blur-[2px] z-10 flex items-center justify-center pointer-events-none">
                  <div className="text-[10px] font-mono text-orange-400/60 uppercase tracking-[0.2em] animate-pulse">Agent Sync Active</div>
              </div>
          )}
          ...
          <div className="mt-4 flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder={aiAgentMode ? "Chatting with bots..." : "Message Eilo..."} className="flex-1 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs outline-none focus:border-cyan-500/30 font-sans" />
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[2000] flex items-center justify-center p-6 text-center font-sans">
          <div className="bg-[#1c1c28] w-full max-w-sm rounded-[45px] p-8 border border-white/10 relative shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar text-left mb-6">
                <p className="text-[10px] uppercase font-bold text-slate-500 mb-2 px-1">Special Agent Modes</p>
                
                {/* AI Agent Mode (Gibberlink) 🤖 */}
                <button onClick={() => setAiAgentMode(!aiAgentMode)} className={`w-full flex items-center justify-between px-5 py-3 rounded-2xl text-xs font-bold border transition-all ${aiAgentMode ? 'bg-orange-600/30 border-orange-500/50 text-orange-200' : 'bg-white/5 border-white/10 text-slate-400'}`}>
                  <div className="flex items-center gap-2"><Radio size={14}/> AI Agent Mode (Gibberlink)</div>
                  {aiAgentMode ? <ToggleRight size={20} className="text-orange-400"/> : <ToggleLeft size={20}/>}
                </button>

                <p className="text-[10px] uppercase font-bold text-slate-500 mt-4 mb-2 px-1">Hardware & Control</p>
                <button onClick={() => setIsChaosMode(!isChaosMode)} className={`w-full flex items-center justify-between px-5 py-3 rounded-2xl text-xs font-bold border transition-all ${isChaosMode ? 'bg-orange-600/30 border-orange-500/50 text-orange-200' : 'bg-white/5 border-white/10 text-slate-400'}`}><div className="flex items-center gap-2"><Ghost size={14}/> Chaos Mode (Legs)</div>{isChaosMode ? <ToggleRight size={20}/> : <ToggleLeft size={20}/>}</button>
                <button onClick={startCamera} className={`w-full flex items-center justify-between px-5 py-3 rounded-2xl text-xs font-bold border transition-all ${visionEnabled ? 'bg-cyan-600/30 border-cyan-500/50 text-cyan-200' : 'bg-white/5 border-white/10 text-slate-400'}`}><div className="flex items-center gap-2"><Eye size={14}/> Selfie Scanner</div>{visionEnabled ? <ToggleRight size={20}/> : <ToggleLeft size={20}/>}</button>
                <button onClick={() => setFearOfHeights(!fearOfHeights)} className={`w-full flex items-center justify-between px-5 py-3 rounded-2xl text-xs font-bold border transition-all ${fearOfHeights ? 'bg-cyan-600/30 border-cyan-500/50 text-cyan-200' : 'bg-white/5 border-white/10 text-slate-400'}`}><div className="flex items-center gap-2"><AlertTriangle size={14}/> Fear of Heights</div>{fearOfHeights ? <ToggleRight size={20}/> : <ToggleLeft size={20}/>}</button>
                
                <p className="text-[10px] uppercase font-bold text-slate-500 mt-4 mb-2 px-1">AI Core Synchronization</p>
                <input type="password" value={tempApiKey} onChange={e => setTempApiKey(e.target.value)} placeholder="Paste Gemini Key..." className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm mb-4 outline-none text-center focus:border-cyan-500/50 text-white" />
            </div>

            <button onClick={() => { localStorage.setItem('eilo_key', tempApiKey); setShowSettings(false); }} className="w-full bg-cyan-600 py-4 rounded-2xl font-bold uppercase text-white shadow-lg active:scale-95 transition-all">Update AI</button>
            <button onClick={() => { signOut(auth); window.location.reload(); }} className="w-full mt-4 text-[10px] text-red-500 font-bold uppercase opacity-50">Disconnect Core</button>
          </div>
        </div>
      )}
      <style dangerouslySetInnerHTML={{ __html: `@keyframes blink { 0%, 95%, 100% { transform: scaleY(1); } 97% { transform: scaleY(0.1); } } .custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(34,211,238,0.2); border-radius: 10px; }`}} />
    </div>
  );
}
