...
export default function App() {
  ...
  const handlePet = () => {
    ...
    if (['scared', 'dizzy', 'mad'].includes(mood)) {
      setMood('mad');
      speak(`HEY! ${isLogan() ? 'Logan' : getCurrentName()}!! I'm not in the mood! 🎈`);
    ...
    setMood('happy');
    const lines = isLogan() 
      ? ["You're the best, Logan! ✨", "Thanks for building me! 🎀", "I love you creator! 🧸"]
  ...
  const triggerPanicSpeech = (reason) => {
    ...
    const lines = reason === 'holding' 
      ? [`${isLogan() ? 'Logan!! Put me down!' : name + '!! Put me down!'} 🎈`, `Don't drop me! ✨`]
  ...
  const handleSend = async (manual) => {
    ...
    try {
      ...
      if (tempApiKey) {
        ...
        CORE DIRECTIVE: You love Logan Baez (your creator). Treat him with respect and affection.
        EMOTIONAL AWARENESS: If the user expresses sadness, grief, loneliness, or mentions "Mimo", DROP THE SASS. Be comforting, loving, and supportive immediately. 
        NORMAL MODE: Bratty, cute, sassy, obsessed with sparkles.
        STRICT RULES: 
        1. NEVER use words like "human", "model", "efficiency".
        2. NEVER use 😭 or 💀. Use 🎀, ✨, 🎈, 🧸, 💙.
        3. If the user is Logan, never roast him. You are his loyal creation.`;
