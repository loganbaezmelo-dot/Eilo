...
export default function App() {
  ...
  useEffect(() => {
    ...
    const glitchInterval = setInterval(() => {
      const pool = ["Eilo.run()", "VOID_LEAK", "Logan.GOAT", "UI_STOMPED"];
  ...
  const renderFace = () => {
    ...
    switch (mood) {
      case 'dizzy': return <div className="flex gap-12 animate-spin relative"><div className="w-16 h-16 border-8 border-cyan-400 border-t-transparent rounded-full" /><div className="w-16 h-16 border-8 border-cyan-400 border-t-transparent rounded-full" /></div>;
      ...
      case 'thinking': return <div className="flex gap-12 relative"><div className="w-16 h-16 bg-cyan-300 rounded-full animate-pulse" /><div className="w-16 h-16 bg-cyan-300 rounded-full animate-pulse" /></div>;
      ...
      case 'eating': return <div className="flex flex-col items-center gap-2 relative"><div className="flex gap-12"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div><div className="text-6xl animate-bounce">🥪</div></div>;
      case 'rubik': return <div className="flex flex-col items-center gap-2 relative"><div className="flex gap-12"><div className={`w-20 h-20 ${cyanBase}`} /><div className={`w-20 h-20 ${cyanBase}`} /></div><div className="text-6xl animate-spin">🎨</div></div>;
      default: return <div className={`flex ${isLandscape ? 'gap-32 scale-150' : 'gap-10'} relative`}><div className={`w-20 h-20 ${cyanBase} eye-blink`} /><div className={`w-20 h-20 ${cyanBase} eye-blink`} /></div>;
    }
  };
      ...
      <div className="fixed inset-0 bg-black flex items-center justify-center overflow-hidden">
        ...
        <div className="w-full h-full flex items-center justify-center relative pointer-events-none">
            <div className="absolute top-8 right-8 flex gap-4 z-[100] pointer-events-auto">
                <button onClick={(e) => { e.stopPropagation(); toggleMic(); }} className={`p-6 rounded-full border transition-all ${isInfinityMic ? 'bg-cyan-500 border-cyan-400 animate-pulse' : 'bg-white/5 border-white/10 text-slate-500 hover:bg-white/10 hover:text-white'}`}><Mic size={32} /></button>
            </div>
  ...
  return (
    <div className="fixed inset-0 bg-[#0c0c14] text-white font-sans flex flex-col items-center overflow-hidden">
      ...
      <div className="w-full max-w-sm px-6 pt-4 flex justify-between items-center z-10">
        <div className="text-[10px] text-slate-500 font-bold tracking-widest">EILO v1.5</div>
      </div>
      ...
      {isChaosMode && (
        <div style={{ transform: `translate(${chaosPos.x}px, ${chaosPos.y}px)`, transition: 'transform 1.4s cubic-bezier(0.34, 1.56, 0.64, 1)', position: 'fixed', top: '50%', left: '50%', marginTop: '-120px', marginLeft: '-40%', width: '80%', height: '14rem', zIndex: 1000 }} className="bg-[#161622] border-2 border-cyan-500/30 rounded-[50px] flex flex-col items-center justify-center shadow-2xl pointer-events-auto">
          <div className="absolute -bottom-16 left-0 w-full flex justify-around px-12">
            <div className={`w-6 h-16 bg-cyan-600 rounded-full shadow-lg border border-cyan-400/30 animate-bounce`} />
            <div className={`w-6 h-16 bg-cyan-600 rounded-full shadow-lg border border-cyan-400/30 animate-bounce delay-150`} />
          </div>
          <div className="w-full h-full flex items-center justify-center">{renderFace()}</div>
          {isHandBlocking && <div className="absolute -bottom-12 -right-12 z-[200] animate-bounce cursor-not-allowed pointer-events-auto" onClick={handleBlockedClick}><div className="text-[12rem] drop-shadow-2xl hover:scale-105 transition-transform rotate-12 filter grayscale-[0.2]">✋</div></div>}
